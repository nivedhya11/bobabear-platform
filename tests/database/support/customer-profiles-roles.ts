/**
 * Application-shaped role for Customer Profile tables (IMP-017).
 * Profiles: SELECT/INSERT/UPDATE/DELETE. Audit: SELECT/INSERT only.
 */
...
export interface CustomerProfileRoleFixture {
...
export async function withCustomerProfileRoleFixture<T>(
  databaseName: string,
  connectionString: string,
  callback: (fixture: CustomerProfileRoleFixture) => Promise<T>,
): Promise<T> {
  const suffix = randomBytes(6).toString("hex");
  const role = `boba_test_cprof_app_${suffix}`;
  ...
  await withTestDatabaseClient(connectionString, async (admin) => {
    ...
    await admin.pool.query(
      `GRANT SELECT, INSERT, UPDATE, DELETE ON app.customer_auth_users TO ${quoteIdentifier(role)}`,
    );
  });
