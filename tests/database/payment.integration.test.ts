...
describe("IMP-022 payment migration inventory", () => {
  it("creates exactly 6 payment tables; prior migrations sealed; 91 app tables", async () => {
    ...
    if (sealedPayment) {
      ...
      expect(integrity.migrations).toHaveLength(17);
    } else {
    ...
    await withPaymentReadyHarness(async ({ persistence }) => {
      await persistence.withContext(async (ctx) => {
        ...
        `);
        expect(appTables.rows[0]?.count).toBe("91");
      });
    });
  });
});
