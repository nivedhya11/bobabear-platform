...
async function login(page: Page) {
  ...
  // Wait for final authenticated hub so MFA verify Set-Cookie commits before Operations navigation.
  // Product copy is "Signed in as {email}" on the workforce applications hub (IMP-036A).
  await expect(page.getByRole("heading", { name: /^Workforce$/i, level: 1 })).toBeVisible();
...
async function assertPostMutationListFreshness(
  ...
  await expect(page.locator("#main-content").getByLabel("Order number", { exact: true })).toBeVisible();
  const statusCell = page.getByTestId(`order-status-${order.id}`);
  if (!(await statusCell.isVisible().catch(() => false))) {
    await page.locator("#main-content").getByLabel("Order number", { exact: true }).fill(order.number);
    await page.locator("#main-content").getByRole("button", { name: "Apply filters" }).click();
  }
...
async function mutate(
  ...
  // Canonical Orders list (Today hub is /workforce/operations/).
  await page.goto("/workforce/operations/orders/");
