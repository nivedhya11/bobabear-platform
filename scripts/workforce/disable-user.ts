...
/**
 * Disable a workforce user and revoke all of their sessions (IMP-010).
 *
 * Usage:
 *   npm run workforce:user:disable -- --email=ops@example.test
 */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    printSafeOk({
      operation: "workforce_user_disable",
...
main().catch((error: unknown) => {
  if (
    ...
    printSafeError("workforce:user:disable failed.");
  }
  process.exitCode = 1;
});
