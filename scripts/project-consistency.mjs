...
/** @param {string} roadmapVersion @param {string} stateVersion @param {"activation" | "lock" | "authorization" | "start" | "routeAmendment" | "consistencyRepair" | "acceptance" | "imp031Activation" | "imp031Draft" | "imp031Lock" | "imp031Authorization" | "imp031Start" | "imp031Completion" | "imp031Acceptance" | "imp032Activation" | "imp032Draft" | "imp032Lock"} [kind] */
export function isSupportedImp030GovernanceCheckpoint(roadmapVersion, stateVersion, kind) {
  ...
  return activation || lock || authorization || start || routeAmendment || consistencyRepair || acceptance || imp031Activation || imp031Draft || imp031Lock || imp031Authorization || imp031Start || imp031Completion || imp031Acceptance || imp032Activation || imp032Draft || imp032Lock;
}
 ...
 * Live CURRENT artifact is ARCHITECTURE_LOCKED under evaluateImp032ArchitectureLockArtifact.
 * @param {string} text
 */
...
function checkTechnicalInventory() {
  ...
  if (!latest || latest.tag !== "0031_delivery") {
    fail(
      "LATEST_MIGRATION",
      `Expected latest migration tag 0031_delivery, got ${latest && latest.tag}`,
    );
  } else {
    note("Latest migration tag 0031_delivery");
  }
  const sqlFiles = readdirSync(path.join(projectRoot, "drizzle")).filter((f) => f.endsWith(".sql"));
  if (sqlFiles.length !== 32 || entries.length !== 32) {
    fail(
      "MIGRATION_COUNT",
      `Expected 32 migrations, got sql=${sqlFiles.length} journal=${entries.length}`,
    );
  } else {
    note("Migration count 32");
  }
  ...
  if (tableCount !== 115) {
    fail("TABLE_COUNT", `Expected 115 appSchema.table declarations, got ${tableCount}`);
  } else {
    note("Application table count 115");
  }
