...
export function isSupportedImp030GovernanceCheckpoint(roadmapVersion, stateVersion, kind) {
  ...
  const imp036dStart = roadmapVersion === "GTM-R106" && stateVersion === "STATE-R104";
  const imp036dCompletion = roadmapVersion === "GTM-R107" && stateVersion === "STATE-R105";
  const imp036dAcceptance = roadmapVersion === "GTM-R108" && stateVersion === "STATE-R106";
  const imp036eActivation = roadmapVersion === "GTM-R109" && stateVersion === "STATE-R107";
  const imp036eLock = roadmapVersion === "GTM-R110" && stateVersion === "STATE-R108";
  ...
  const imp036eAcceptance = roadmapVersion === "GTM-R115" && stateVersion === "STATE-R113";
  ...
  if (kind === "imp036dStart") return imp036dStart;
  if (kind === "imp036dCompletion") return imp036dCompletion;
  if (kind === "imp036dAcceptance") return imp036dAcceptance;
  if (kind === "imp036eActivation") return imp036eActivation;
  if (kind === "imp036eLock") return imp036eLock;
  ...
  if (kind === "imp036eAcceptance") return imp036eAcceptance;
...
function isImp036dImplementationStartCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036dStart");
}

function isImp036dImplementationCompletionCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036dCompletion");
}

function isImp036dAcceptanceCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036dAcceptance");
}

function isImp036eArchitectureActivationCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036eActivation");
}

function isImp036eArchitectureLockCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036eLock");
}

function isImp036eImplementationAuthorizationCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036eAuthorization");
}

function isImp036eImplementationStartCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036eStart");
}

function isImp036eImplementationCompletionCheckpoint(roadmap, state) {
...
function isImp036eAcceptanceCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036eAcceptance");
}

function isImp036fActivationCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(roadmap?.meta.roadmapVersion, state?.meta.stateVersion, "imp036fActivation");
}

function isImp036fProductDefinitionDraftAuthorizedCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(
    roadmap?.meta.roadmapVersion,
    state?.meta.stateVersion,
    "imp036fProductDefinitionDraftAuthorized",
  );
}

function isImp036fProductDefinitionGatePassCheckpoint(roadmap, state) {
  return isSupportedImp030GovernanceCheckpoint(
    roadmap?.meta.roadmapVersion,
    state?.meta.stateVersion,
    "imp036fProductDefinitionGatePass",
  );
}
...
function isImp030GovernanceCheckpoint(roadmap, state) {
  return (
    ...
    isImp036dImplementationStartCheckpoint(roadmap, state) ||
    isImp036dImplementationCompletionCheckpoint(roadmap, state) ||
    isImp036dAcceptanceCheckpoint(roadmap, state) ||
    isImp036eArchitectureActivationCheckpoint(roadmap, state) ||
...
/**
 * Validate the exact IMP-036E architecture-activation lifecycle facts (R109/S107).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eArchitectureActivationCheckpoint(checkpoint) {
  const expected = {
    ...
    pendingAcceptance: "NONE",
    imp036d: "COMPLETE_AND_ACCEPTED",
    imp036e: "ARCHITECTURE_IN_PROGRESS",
    architecture: "NOT_LOCKED",
    architectureLocked: "NO",
    implementation: "NOT_AUTHORIZED / NOT_STARTED",
    implementationAuthorized: "NO",
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036E_ARCHITECTURE_ACTIVATION", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E architecture activation" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E architecture activation" };
  }
  if (checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_ARTIFACT",
      message: "IMP-036E capability artifact must not be created during architecture activation",
    };
  }
  if (checkpoint.imp036dAccepted !== true) {
    return { ok: false, code: "IMP036E_IMP036D_ACCEPTANCE", message: "IMP-036D must remain COMPLETE_AND_ACCEPTED" };
  }
  if (checkpoint.serviceabilityAligned !== true) {
    return {
      ok: false,
      code: "IMP036E_SERVICEABILITY_ALIGNMENT",
      message: "IMP-036E experience contract must align to OUTLET_DISTANCE_SERVICEABILITY_V1",
    };
  }
  return { ok: true };
}
...
/**
 * Validate the exact IMP-036E architecture-lock lifecycle facts (R110/S108).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eArchitectureLockCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036E_ARCHITECTURE_LOCK", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E architecture lock" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E architecture lock" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_ARTIFACT",
      message: "IMP-036E locked capability artifact must exist",
    };
  }
  if (!checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_LOCK",
      message: "IMP-036E capability artifact must record ARCHITECTURE_LOCKED / NOT_AUTHORIZED",
    };
  }
  if (checkpoint.imp036dAccepted !== true) {
    return { ok: false, code: "IMP036E_IMP036D_ACCEPTANCE", message: "IMP-036D must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the locked IMP-036E capability artifact without accepting authorization/start progression.
 * @param {string} text
 */
export function evaluateImp036eArchitectureLockArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    ...
    /IMP-036E_ARCHITECTURE:\s*LOCKED/,
    /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/,
    /IMP-036E_STARTED:\s*NO/,
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/,
    /IMP-036E_ACCEPTED:\s*NO/,
    /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
    ...
    /D-374_CREATED:\s*NO/,
    /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK:\s*NO/,
    /NEW_PERMISSION:\s*NO/,
    /NEW_ROLE:\s*NO/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_LOCK", message: "IMP-036E artifact must record the complete ARCHITECTURE_LOCKED checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*true/,
    /IMP-036E_ARCHITECTURE_LOCKED:\s*NO/,
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036E_STARTED:\s*YES/,
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/,
...
/**
 * Validate the exact IMP-036E implementation-authorization lifecycle facts (R111/S109).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eImplementationAuthorizationCheckpoint(checkpoint) {
  const expected = {
    roadmapVersion: "GTM-R111",
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036E_IMPLEMENTATION_AUTHORIZATION", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E implementation authorization" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E implementation authorization" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
  ...
  if (!checkpoint.capabilityArtifactValid && !checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_AUTHORIZATION",
      message: "IMP-036E capability artifact must record ARCHITECTURE_LOCKED / AUTHORIZED / NOT_STARTED",
    };
  }
  if (checkpoint.imp036dAccepted !== true) {
    return { ok: false, code: "IMP036E_IMP036D_ACCEPTANCE", message: "IMP-036D must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the authorized-not-started IMP-036E capability artifact without accepting start progression.
 * @param {string} text
 */
export function evaluateImp036eImplementationAuthorizationArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    /"capability":\s*"IMP-036E"/,
    ...
    /IMP-036E_ARCHITECTURE:\s*LOCKED/,
    /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ NOT_STARTED/,
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036E_STARTED:\s*NO/,
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/,
    /IMP-036E_ACCEPTED:\s*NO/,
    /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
    /AUTHORIZATION IS NOT IMPLEMENTATION START:\s*YES/,
    /Founder implementation authorization recorded at\s*GTM-R111 \/ STATE-R109/,
    ...
    /D-374_CREATED:\s*NO/,
    /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK:\s*NO/,
    /ARCH_R20_CREATED:\s*NO|ARCH-R20_CREATED:\s*NO/,
    /NEW_PERMISSION:\s*NO/,
    /NEW_ROLE:\s*NO/,
    /NEW_SCOPE_MODEL:\s*NO/,
    /GET\s+\/api\/operations\/v1\/outlets\/\{outletId\}\/assortment/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_AUTHORIZATION", message: "IMP-036E artifact must record the complete AUTHORIZED / NOT_STARTED checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*false/,
    ...
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/,
    /IMP-036E_ACCEPTED:\s*YES/,
    /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/,
    /\|\s*D-374\s*\|/,
    /###\s*D-374\b/,
    /IMP036E_GLOBAL_SESSION_CAPS_ARE_RESOURCE_AUTHORITY\s*=\s*YES/,
    /ASSORTMENT_MANAGE_ROUTE_IMP036E\s*=\s*YES/,
    ...
    /PATCH\s+\/api\/operations\/v1\/outlets\/\{outletId\}\/assortment/,
    /DELETE\s+\/api\/operations\/v1\/outlets\/\{outletId\}\/assortment/,
    /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY\s*=\s*YES/,
  ];
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_PROGRESSION", message: "IMP-036E authorization must not claim start, acceptance, unlock, unauthorized state, Assortment manage transport, PIN authority, or D-374" };
  }
  return { ok: true };
}

/**
 * Validate the exact IMP-036E implementation-start lifecycle facts (R112/S110).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eImplementationStartCheckpoint(checkpoint) {
  const expected = {
    roadmapVersion: "GTM-R112",
    stateVersion: "STATE-R110",
    acceptedThrough: "IMP-036D",
    currentProductSlice: "IMP-036E",
    nextProductSlice: "IMP-036F",
    pendingAcceptance: "NONE",
    currentProductImplementation: "IMP-036E",
    imp036d: "COMPLETE_AND_ACCEPTED",
    imp036e: "IMPLEMENTATION_IN_PROGRESS",
    architecture: "LOCKED",
    architectureLocked: "YES",
    ...
    founderUatRequired: "YES",
    imp036f: "PLANNED",
    roadmapLifecycle: "IMPLEMENTATION_IN_PROGRESS",
    stateLifecycle: "IMPLEMENTATION_IN_PROGRESS",
    architectureVersion: "ARCH-R19",
    decisionRegisterVersion: "DR-15",
    schemaChangeRequired: "NO",
  };
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036E_IMPLEMENTATION_START", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E implementation start" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E implementation start" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
  ...
  if (!checkpoint.capabilityArtifactValid && !checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_START",
      message: "IMP-036E capability artifact must record ARCHITECTURE_LOCKED / AUTHORIZED / STARTED",
    };
  }
  if (checkpoint.imp036dAccepted !== true) {
    return { ok: false, code: "IMP036E_IMP036D_ACCEPTANCE", message: "IMP-036D must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the authorized-started IMP-036E capability artifact without accepting completion.
 * @param {string} text
 */
export function evaluateImp036eImplementationStartArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    /"capability":\s*"IMP-036E"/,
    ...
    /IMP-036E_ARCHITECTURE:\s*LOCKED/,
    /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/,
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036E_STARTED:\s*YES/,
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/,
    /IMP-036E_ACCEPTED:\s*NO/,
    /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
    /IMPLEMENTATION_START_RECORDED:\s*YES/,
    /Founder implementation-start authorization recorded at\s*GTM-R112 \/ STATE-R110/,
    /This checkpoint contains no product implementation itself/,
    /IMP036E_ASSORTMENT_AUTHORITY\s*=\s*BRAND/,
    /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE\s*=\s*NO/,
    /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION\s*=\s*AUTHORIZED_READ_OR_ESCALATE/,
    /IMP036E_ASSORTMENT_WORKFORCE_TRANSPORT\s*=\s*READ_ONLY_OPERATIONS_PROJECTION/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_START", message: "IMP-036E artifact must record the complete AUTHORIZED / STARTED checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*false/,
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_PROGRESSION", message: "IMP-036E start must not claim completion, acceptance, unlock, unauthorized/not-started state, Assortment manage transport, PIN authority, or D-374" };
  }
  return { ok: true };
}

/**
 * Validate the exact IMP-036E implementation-completion lifecycle facts (R113/S111).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eImplementationCompletionCheckpoint(checkpoint) {
  const expected = {
    roadmapVersion: "GTM-R113",
    stateVersion: "STATE-R111",
    acceptedThrough: "IMP-036D",
    currentProductSlice: "IMP-036E",
    nextProductSlice: "IMP-036F",
    ...
    currentProductImplementation: "IMP-036E",
    imp036d: "COMPLETE_AND_ACCEPTED",
    imp036e: "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    architecture: "LOCKED",
    architectureLocked: "YES",
    ...
    imp036f: "PLANNED",
    roadmapLifecycle: "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    stateLifecycle: "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    architectureVersion: "ARCH-R19",
    decisionRegisterVersion: "DR-15",
    schemaChangeRequired: "NO",
  };
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036E_IMPLEMENTATION_COMPLETION", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E implementation completion" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E implementation completion" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
  ...
  if (!checkpoint.capabilityArtifactValid && !checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036E_CAPABILITY_COMPLETION",
      message: "IMP-036E capability artifact must record ARCHITECTURE_LOCKED / AUTHORIZED / STARTED / COMPLETE pending acceptance",
    };
  }
  if (checkpoint.imp036dAccepted !== true) {
    return { ok: false, code: "IMP036E_IMP036D_ACCEPTANCE", message: "IMP-036D must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the completed-pending-acceptance IMP-036E capability artifact without accepting.
 * @param {string} text
 */
export function evaluateImp036eImplementationCompletionArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    /"capability":\s*"IMP-036E"/,
    ...
    /"founderUATRequired":\s*true/,
    /"schemaChangeRequired":\s*false/,
    /IMP-036E:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/,
    /IMP-036E_ARCHITECTURE:\s*LOCKED/,
    /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/,
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036E_STARTED:\s*YES/,
    /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/,
    /IMP-036E_ACCEPTED:\s*NO/,
    /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
    /IMP-036E_FOUNDER_UAT:\s*NOT_STARTED/,
    /COMPLETION IS NOT ACCEPTANCE:\s*YES/,
    /Implementation complete pending acceptance recorded at\s*GTM-R113 \/ STATE-R111/,
    /IMP036E_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/,
    /IMP_036E_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/,
    /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/,
    /IMP036E_IMPLEMENTATION_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/,
    /IMP036E_REVIEWED_CANDIDATE_HEAD:\s*b2ffaa5bc2b5c6f58ff5241c226b583160132837/,
    /IMP036E_REVIEWED_CANDIDATE_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/,
    /IMP036E_ASSORTMENT_AUTHORITY\s*=\s*BRAND/,
    /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE\s*=\s*NO/,
    /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION\s*=\s*AUTHORIZED_READ_OR_ESCALATE/,
    /IMP036E_ASSORTMENT_WORKFORCE_TRANSPORT\s*=\s*READ_ONLY_OPERATIONS_PROJECTION/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_COMPLETION", message: "IMP-036E artifact must record the complete IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*false/,
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_PROGRESSION", message: "IMP-036E completion must not claim acceptance, Founder UAT PASS, incomplete state, Assortment manage transport, PIN authority, or D-374" };
  }
  return { ok: true };
}
...
/**
 * Validate the exact IMP-036D implementation-start lifecycle facts (R106/S104).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036dImplementationStartCheckpoint(checkpoint) {
  const expected = {
    ...
    pendingAcceptance: "NONE",
    imp036c: "COMPLETE_AND_ACCEPTED",
    imp036d: "IMPLEMENTATION_IN_PROGRESS",
    architecture: "LOCKED",
    architectureLocked: "YES",
    implementation: "AUTHORIZED / STARTED",
    implementationAuthorized: "YES",
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036D_IMPLEMENTATION_START", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036D_D374", message: "D-374 must not be created during IMP-036D implementation start" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
  ...
  if (!checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036D_CAPABILITY_START",
      message: "IMP-036D capability artifact must record ARCHITECTURE_LOCKED / AUTHORIZED / STARTED",
    };
  }
  if (checkpoint.imp036cAccepted !== true) {
    return { ok: false, code: "IMP036D_IMP036C_ACCEPTANCE", message: "IMP-036C must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the authorized-started IMP-036D capability artifact without accepting completion.
 * @param {string} text
 */
export function evaluateImp036dImplementationStartArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    /"capability":\s*"IMP-036D"/,
    ...
    /IMP-036D_ARCHITECTURE:\s*LOCKED/,
    /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/,
    /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036D_STARTED:\s*YES/,
    /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/,
    /IMP-036D_ACCEPTED:\s*NO/,
    /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/,
    /START IS NOT COMPLETION OR ACCEPTANCE:\s*YES/,
    /Founder implementation-start authorization recorded at\s*GTM-R106 \/ STATE-R104/,
    /Start does not\s*complete or accept implementation/,
    /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/,
    /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*NO/,
    /REFUND_WORKFORCE_TRANSPORT\s*=\s*OPERATIONS_PROCESS/,
    /REFUND_PROVIDER_EXECUTION\s*=\s*CUSTOMER_COMMERCE/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036D_CAPABILITY_START", message: "IMP-036D artifact must record the complete AUTHORIZED / STARTED checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*false/,
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036D_CAPABILITY_PROGRESSION", message: "IMP-036D start must not claim completion, acceptance, unlock, not-started, unresolved refund topology, or D-374" };
  }
  return { ok: true };
}

/**
 * Validate the exact IMP-036D implementation-completion lifecycle facts (R107/S105).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036dImplementationCompletionCheckpoint(checkpoint) {
  const expected = {
    ...
    imp036c: "COMPLETE_AND_ACCEPTED",
    imp036d: "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    architecture: "LOCKED",
    architectureLocked: "YES",
    implementation: "AUTHORIZED / STARTED / COMPLETE",
    implementationAuthorized: "YES",
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036D_IMPLEMENTATION_COMPLETION", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036D_D374", message: "D-374 must not be created during IMP-036D implementation completion" };
  }
  if (!checkpoint.capabilityArtifactExists) {
    return {
      ok: false,
  ...
  if (!checkpoint.artifactValid) {
    return {
      ok: false,
      code: "IMP036D_CAPABILITY_COMPLETION",
      message: "IMP-036D capability artifact must record ARCHITECTURE_LOCKED / AUTHORIZED / STARTED / COMPLETE pending acceptance",
    };
  }
  if (checkpoint.imp036cAccepted !== true) {
    return { ok: false, code: "IMP036D_IMP036C_ACCEPTANCE", message: "IMP-036C must remain COMPLETE_AND_ACCEPTED" };
...
/**
 * Validate the completed-pending-acceptance IMP-036D capability artifact without accepting.
 * @param {string} text
 */
export function evaluateImp036dImplementationCompletionArtifact(text) {
  const required = [
    /"status":\s*"CURRENT"/,
    /"authority":\s*"CAPABILITY_ARCHITECTURE"/,
    /"capability":\s*"IMP-036D"/,
    ...
    /"founderUATRequired":\s*true/,
    /"schemaChangeRequired":\s*false/,
    /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/,
    /IMP-036D_ARCHITECTURE:\s*LOCKED/,
    /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/,
    /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036D_STARTED:\s*YES/,
    /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/,
    /IMP-036D_ACCEPTED:\s*NO/,
    /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/,
    /IMP-036D_FOUNDER_UAT:\s*NOT_STARTED/,
    /COMPLETION IS NOT ACCEPTANCE:\s*YES/,
    /Implementation complete pending acceptance recorded at\s*GTM-R107 \/ STATE-R105/,
    /IMP036D_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/,
    /IMP_036D_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/,
    /IMP036D_IMPLEMENTATION_MERGE_SHA:\s*b615fb20e034d71f49a1fa588f976d0592a46098/,
    /IMP036D_IMPLEMENTATION_TREE:\s*8a5e75516ccee922e89c6202ad402e5a2acb24b9/,
    /IMP036D_REVIEWED_CANDIDATE_HEAD:\s*72e321ffd90d4d6f009ee1457bb728b22fc0a2fd/,
    /IMP036D_REVIEWED_CANDIDATE_TREE:\s*8a5e75516ccee922e89c6202ad402e5a2acb24b9/,
    /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/,
    /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*NO/,
    /REFUND_WORKFORCE_TRANSPORT\s*=\s*OPERATIONS_PROCESS/,
    /REFUND_PROVIDER_EXECUTION\s*=\s*CUSTOMER_COMMERCE/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036D_CAPABILITY_COMPLETION", message: "IMP-036D artifact must record the complete IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE checkpoint" };
  }
  const forbidden = [
    /"architectureLock":\s*"NOT_LOCKED"/,
    /"implementationAuthorized":\s*false/,
    ...
    /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/,
    /IMP-036D_ACCEPTED:\s*YES/,
    /IMP-036D:\s*COMPLETE_AND_ACCEPTED/,
    /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/,
    /IMP-036D_FOUNDER_UAT:\s*PASS/,
    /START IS NOT COMPLETION OR ACCEPTANCE:\s*YES/,
    /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*DECISION_REQUIRED/,
    /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*YES/,
    /\|\s*D-374\s*\|/,
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036D_CAPABILITY_PROGRESSION", message: "IMP-036D completion must not claim acceptance, Founder UAT PASS, incomplete state, unresolved refund topology, or D-374" };
  }
  return { ok: true };
}

/**
 * Validate the exact IMP-036D acceptance lifecycle facts (R108/S106).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036dAcceptanceCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) return { ok: false, code: "IMP036D_ACCEPTANCE", message: `${key} must be ${value}` };
  }
  if (!checkpoint.artifact) {
    return { ok: false, code: "IMP036D_CAPABILITY_MISSING", message: "IMP-036D locked capability artifact must exist and record acceptance" };
  }
  if (!checkpoint.founderUatPass) {
    return { ok: false, code: "IMP036D_FOUNDER_UAT", message: "IMP-036D must record FOUNDER_UAT: PASS" };
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036D_D374", message: "D-374 must not be created during IMP-036D acceptance" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036D_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036D acceptance" };
  }
  return { ok: true };
}

/**
 * Validate the accepted IMP-036D capability artifact.
 * @param {string} text
 */
export function evaluateImp036dAcceptanceArtifact(text) {
  const required = [
    /"capability":\s*"IMP-036D"/,
    /IMP-036D:\s*COMPLETE_AND_ACCEPTED/,
    /IMP-036D_ACCEPTED:\s*YES/,
    /IMP-036D_FOUNDER_UAT:\s*PASS/,
    /IMP036D_FORMAL_ACCEPTANCE:\s*ACCEPTED/,
    /IMP036D_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/,
    /IMP036D_ACCEPTED_MAIN_SHA:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/,
    /IMP036D_ACCEPTED_TREE:\s*6580497091525c3ddd892aa44aafd63ef1132d35/,
    /IMP036D_ACCEPTED_CANDIDATE:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/,
    /IMP036D_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*3d999b0a154e70b9332d8f5228d1723ddd501cf8b6ec162ebec8c1bfc042a735/,
    /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/,
    /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*NO/,
    /IMP036D_REFUND_MUTATION_TRANSPORT_LOCKED:\s*YES/,
    /IMP036D_PREPARATION_READINESS_DECISION:\s*NO_NEW_V1_DOMAIN_STATE_REQUIRED/,
    /IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW:\s*DEFERRED/,
    /SCHEMA_CHANGE_REQUIRED:\s*NO/,
    /D-374_CREATED:\s*NO/,
  ...
  if (required.some((pattern) => !pattern.test(text))) {
    return { ok: false, code: "IMP036D_CAPABILITY_ACCEPTANCE", message: "IMP-036D artifact must record COMPLETE_AND_ACCEPTED" };
  }
  return { ok: true };
}
...
/**
 * Validate the exact IMP-036E acceptance lifecycle facts (R115/S113).
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036eAcceptanceCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) return { ok: false, code: "IMP036E_ACCEPTANCE", message: `${key} must be ${value}` };
  }
  if (!checkpoint.artifact) {
    return { ok: false, code: "IMP036E_CAPABILITY_MISSING", message: "IMP-036E locked capability artifact must exist and record acceptance" };
  }
  if (!checkpoint.founderUatPass) {
    return { ok: false, code: "IMP036E_FOUNDER_UAT", message: "IMP-036E must record FOUNDER_UAT: PASS" };
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036E_D374", message: "D-374 must not be created during IMP-036E acceptance" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036E_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036E acceptance" };
  }
  if (checkpoint.imp036fActivated !== false) {
    return { ok: false, code: "IMP036E_IMP036F_ACTIVATION", message: "IMP-036F must remain unactivated during IMP-036E acceptance" };
  }
  return { ok: true };
}

/**
 * Validate the accepted IMP-036E capability artifact.
 * @param {string} text
 */

/**
 * Validate IMP-036F product-slice ADVANCE / activation (R116/S114).
 * Formal ROADMAP lifecycle remains PLANNED; activation is IMP036F_ACTIVATED + currentProductSlice.
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036fActivationCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) return { ok: false, code: "IMP036F_ACTIVATION", message: `${key} must be ${value}` };
  }
  if (checkpoint.productDefinitionExists) {
    return { ok: false, code: "IMP036F_PRODUCT_DEFINITION", message: "IMP-036F Product Definition must not exist at activation" };
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036F_D374", message: "D-374 must not be created during IMP-036F activation" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036F_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036F activation" };
  }
  return { ok: true };
}

/**
 * Validate IMP-036F post-discovery Product Definition draft authorization (R117/S115).
 * Formal ROADMAP lifecycle remains PLANNED; Product Definition file may be absent or an ungated draft.
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036fProductDefinitionDraftAuthorizedCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036F_PD_DRAFT_AUTHORIZED", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036F_D374", message: "D-374 must not be created during IMP-036F draft authorization" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036F_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036F draft authorization" };
  }
  if (checkpoint.productDefinitionExists) {
    const draft = evaluateImp036fUngatedProductDefinitionDraftCandidate(
...
/**
 * Mechanical validation for an ungated IMP-036F Product Definition draft candidate.
 * Does not semantically evaluate the full Product Definition.
 * @param {string} text
 */
export function evaluateImp036fUngatedProductDefinitionDraftCandidate(text) {
  if (!text || !String(text).trim()) {
    return { ok: false, code: "IMP036F_PD_DRAFT_EMPTY", message: "Present IMP-036F Product Definition draft must not be empty" };
  }
  ...
  if (
    ...
    /"architectureLock"\s*:\s*"ARCHITECTURE_LOCKED"/.test(body) ||
    /architecture\s*[:=]\s*LOCKED\b/i.test(body)
  ) {
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_ARCHITECTURE",
      message: "Ungated IMP-036F Product Definition candidate must not claim architecture locked or architecture fit performed",
    };
  }
  if (
    ...
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_IMPLEMENTATION",
      message: "Ungated IMP-036F Product Definition candidate must not authorize or start implementation",
    };
  }
  if (/IMP036F_ACCEPTED\s*[:=]\s*YES/.test(body) || /IMP-036F\s*[:=]\s*COMPLETE_AND_ACCEPTED/.test(body)) {
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_ACCEPTANCE",
      message: "Ungated IMP-036F Product Definition candidate must not claim IMP acceptance",
    };
  }
  if (/IMP036G_ACTIVATED\s*[:=]\s*YES/.test(body) || /IMP-036G\s*[:=]\s*ACTIVATED/.test(body)) {
    return {
      ok: false,
      code: "IMP036F_PD_IMP036G_ACTIVATION",
      message: "Ungated IMP-036F Product Definition candidate must not activate IMP-036G",
    };
  }
  return { ok: true };
}
...
/**
 * Validate IMP-036F post-Product-Definition-Gate PASS checkpoint (R118/S116).
 * Formal ROADMAP lifecycle remains PLANNED; approved PD file is required; Architecture Fit not performed.
 * @param {Record<string, unknown>} checkpoint
 */
export function evaluateImp036fProductDefinitionGatePassCheckpoint(checkpoint) {
  const expected = {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (checkpoint[key] !== value) {
      return { ok: false, code: "IMP036F_PD_GATE_PASS", message: `${key} must be ${value}` };
    }
  }
  if (checkpoint.d374Exists) {
    return { ok: false, code: "IMP036F_D374", message: "D-374 must not be created during IMP-036F Product Definition Gate PASS persistence" };
  }
  if (checkpoint.archR20Exists) {
    return { ok: false, code: "IMP036F_ARCH_R20", message: "ARCH-R20 must not be created during IMP-036F Product Definition Gate PASS persistence" };
  }
  const approved = evaluateImp036fApprovedProductDefinitionCandidate(
    typeof checkpoint.productDefinitionText === "string" ? checkpoint.productDefinitionText : "",
  );
...
/**
 * Mechanical validation for an approved / gate-passed IMP-036F Product Definition.
 * Does not semantically re-evaluate product behaviour.
 * @param {string} text
 */
export function evaluateImp036fApprovedProductDefinitionCandidate(text) {
  if (!text || !String(text).trim()) {
    return { ok: false, code: "IMP036F_PD_APPROVED_ABSENT", message: "Approved IMP-036F Product Definition must not be empty" };
  }
  const body = String(text);
  const hasApprovedStatus =
    /"status"\s*:\s*"APPROVED"/i.test(body) ||
    /Document status\s*[:=]\s*APPROVED/i.test(body) ||
    /Product Definition version \/ document status\s*\|\s*[^|]*\bAPPROVED\b/i.test(body) ||
    /\bdocument status\b[^\n]*\bAPPROVED\b/i.test(body) ||
    /\bstatus\b\s*[:=]\s*APPROVED\b/i.test(body);
  if (!hasApprovedStatus) {
    return {
      ok: false,
      code: "IMP036F_PD_APPROVED_STATUS",
      message: "Gate-passed IMP-036F Product Definition must record Document status = APPROVED",
    };
  }
  if (/Document status\s*[:=]\s*DRAFT\b/i.test(body) || /"status"\s*:\s*"DRAFT"/i.test(body)) {
    return {
      ok: false,
      code: "IMP036F_PD_STALE_DRAFT",
      message: "Gate-passed IMP-036F Product Definition must not retain Document status DRAFT",
    };
  }
  const hasExecutionPerformed = /PRODUCT_DEFINITION_GATE_EXECUTION\s*[:=]\s*PERFORMED/.test(body);
  const hasGateResultPass = /Gate Result\s*[:=]\s*PASS\b/.test(body);
  ...
  if (!hasExecutionPerformed) {
    return {
      ok: false,
      code: "IMP036F_PD_GATE_EXECUTION_PERFORMED",
      message: "Gate-passed IMP-036F Product Definition must record PRODUCT_DEFINITION_GATE_EXECUTION: PERFORMED",
    };
  }
  if (hasGateResultStop) {
    return {
      ok: false,
      code: "IMP036F_PD_GATE_RESULT_STOP",
      message: "R118/S116 PASS checkpoint rejects Gate Result: STOP",
    };
  }
  if (!hasGateResultPass) {
    return {
      ok: false,
      code: "IMP036F_PD_GATE_RESULT_PASS",
      message: "Gate-passed IMP-036F Product Definition must record Gate Result: PASS",
    };
  }
  if (hasExecutionNotPerformed || hasGateResultNotPerformed) {
    return {
      ok: false,
      code: "IMP036F_PD_GATE_STALE_NOT_PERFORMED",
      message: "Gate-passed IMP-036F Product Definition must not retain gate NOT_PERFORMED markers",
    };
  }
  if (
    !/ARCHITECTURE_FIT\s*[:=]\s*NOT_PERFORMED/.test(body) &&
    !/"architectureFit"\s*:\s*"NOT_PERFORMED"/.test(body)
  ) {
    return {
      ok: false,
      code: "IMP036F_PD_ARCHITECTURE_FIT_NOT_PERFORMED",
      message: "Gate-passed IMP-036F Product Definition must record ARCHITECTURE_FIT: NOT_PERFORMED",
    };
  }
  if (
    /ARCHITECTURE_FIT\s*[:=]\s*(PERFORMED|PASS|LOCKED)\b/i.test(body) ||
    /architecture fit\s*[:=]\s*(performed|pass|locked)/i.test(body) ||
    /IMP036F_ARCHITECTURE_LOCKED\s*[:=]\s*YES/.test(body) ||
    /"architectureLock"\s*:\s*"ARCHITECTURE_LOCKED"/.test(body) ||
    /architecture\s*[:=]\s*LOCKED\b/i.test(body)
    ...
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_ARCHITECTURE",
      message: "Gate-passed IMP-036F Product Definition must not claim Architecture Fit complete or architecture locked",
    };
  }
  if (
    /IMP036F_IMPLEMENTATION_AUTHORIZED\s*[:=]\s*YES/.test(body) ||
    ...
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_IMPLEMENTATION",
      message: "Gate-passed IMP-036F Product Definition must not authorize or start implementation",
    };
  }
  if (/IMP036F_ACCEPTED\s*[:=]\s*YES/.test(body) || /IMP-036F\s*[:=]\s*COMPLETE_AND_ACCEPTED/.test(body)) {
    return {
      ok: false,
      code: "IMP036F_PD_PREMATURE_ACCEPTANCE",
      message: "Gate-passed IMP-036F Product Definition must not claim IMP acceptance",
    };
  }
  if (/IMP036G_ACTIVATED\s*[:=]\s*YES/.test(body) || /IMP-036G\s*[:=]\s*ACTIVATED/.test(body)) {
    return {
      ok: false,
      code: "IMP036F_PD_IMP036G_ACTIVATION",
      message: "Gate-passed IMP-036F Product Definition must not activate IMP-036G",
    };
  }
  return { ok: true };
}
...
export function evaluateImp036eAcceptanceArtifact(text) {
  const required = [
    ...
    /IMP-036E_ACCEPTED:\s*YES/,
    /IMP-036E_FOUNDER_UAT:\s*PASS/,
    /IMP036E_FORMAL_ACCEPTANCE:\s*ACCEPTED/,
    /IMP036E_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/,
    /IMP036E_ACCEPTED_MAIN_SHA:\s*05c534bac3d077f5ab89928495568bb63faf78df/,
    /IMP036E_ACCEPTED_TREE:\s*55b28977ee9860c2c07cb25f751c9f48ef4a2aa6/,
    /IMP036E_ACCEPTED_CANDIDATE:\s*05c534bac3d077f5ab89928495568bb63faf78df/,
    /IMP036E_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*1a97d3a4c80394804e19398e4f3684067aefb0d42a01a7899e8777d1a07cb289/,
    /IMP036E_FOUNDER_UAT_DECISION_DATE:\s*2026-09-10/,
    /IMP036E_FOUNDER_UAT_ACCEPTANCE_AUTHORITY:\s*Founder/,
    /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/,
    /IMP036E_IMPLEMENTATION_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/,
    /IMP036E_REVIEWED_CANDIDATE_HEAD:\s*b2ffaa5bc2b5c6f58ff5241c226b583160132837/,
    /IMP036E_ASSORTMENT_AUTHORITY\s*[:=]\s*BRAND/,
    ...
    /SCHEMA_CHANGE_REQUIRED:\s*NO/,
    /D-374_CREATED:\s*NO/,
    /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK:\s*NO|ARCH_R20_CREATED:\s*NO|ARCH-R20_CREATED:\s*NO/,
    /NEW_PERMISSION:\s*NO/,
    /NEW_ROLE:\s*NO/,
    /NEW_SCOPE_MODEL:\s*NO/,
  ...
  if (forbidden.some((pattern) => pattern.test(text))) {
    return { ok: false, code: "IMP036E_CAPABILITY_PROGRESSION", message: "IMP-036E acceptance must not retain pending-acceptance claims or activate IMP-036F / D-374" };
  }
  return { ok: true };
}
...
function checkImp036dImplementationStart(roadmap, state, architecture, decision) {
  if (!isImp036dImplementationStartCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036dImplementationStartArtifact(artifactText);
  const artifactValid = artifact !== null && artifactValidation.ok;
  ...
  const requiredTokens = [
    [currentRoadmapSection, /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/, "ROADMAP must record IMP-036D IMPLEMENTATION_IN_PROGRESS"],
    [currentRoadmapSection, /IMP-036D_ARCHITECTURE:\s*LOCKED/, "ROADMAP must record IMP-036D architecture LOCKED"],
    [currentRoadmapSection, /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/, "ROADMAP must record IMP-036D architecture lock YES"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/, "ROADMAP must record IMP-036D AUTHORIZED / STARTED"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/, "ROADMAP must record IMP-036D implementation authorized"],
    [currentRoadmapSection, /IMP-036D_STARTED:\s*YES/, "ROADMAP must record IMP-036D started"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/, "ROADMAP must record IMP-036D incomplete"],
    [currentRoadmapSection, /IMP-036D_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036D unaccepted"],
    [currentRoadmapSection, /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036D Founder UAT required"],
    [currentRoadmapSection, /Start does \*\*not\*\* complete or\s+accept implementation/, "ROADMAP must state start does not complete or accept"],
    [currentRoadmapSection, /IMP036D_PREPARATION_READINESS_DECISION:\s*NO_NEW_V1_DOMAIN_STATE_REQUIRED/, "ROADMAP must record preparation/readiness decision"],
    [currentRoadmapSection, /IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW:\s*DEFERRED/, "ROADMAP must defer FD workforce review"],
    [currentRoadmapSection, /IMP036D_NOTIFICATION_RESEND_WORKFORCE_TRANSPORT:\s*APPROVED_FOR_ARCHITECTURE/, "ROADMAP must record notification resend architecture direction"],
    [currentRoadmapSection, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "ROADMAP must record refund topology RESOLVED_AND_LOCKED"],
    ...
    [currentRoadmapSection, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED/, "ROADMAP must keep IMP-036E PLANNED / NOT_ACTIVATED"],
    [currentRoadmapSection, /IMP-036C:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036C COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/, "STATE must record IMP-036D IMPLEMENTATION_IN_PROGRESS"],
    [currentStateAcceptance, /IMP-036D_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036D architecture LOCKED"],
    [currentStateAcceptance, /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036D architecture lock YES"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/, "STATE must record IMP-036D AUTHORIZED / STARTED"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/, "STATE must record IMP-036D implementation authorized"],
    [currentStateAcceptance, /IMP-036D_STARTED:\s*YES/, "STATE must record IMP-036D started"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/, "STATE must record IMP-036D incomplete"],
    [currentStateAcceptance, /IMP-036D_ACCEPTED:\s*NO/, "STATE must record IMP-036D unaccepted"],
    [currentStateAcceptance, /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036D Founder UAT required"],
    [currentStateAcceptance, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "STATE must record refund topology RESOLVED_AND_LOCKED"],
    ...
    [currentStateAcceptance, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036E PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036C:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036C COMPLETE_AND_ACCEPTED"],
    [currentStateActivity, /IMP-036D IMPLEMENTATION_IN_PROGRESS/, "STATE current governance activity must record IMP-036D IMPLEMENTATION_IN_PROGRESS"],
    [currentStateActivity, /AUTHORIZED \/ STARTED/, "STATE current governance activity must record AUTHORIZED / STARTED"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036D_IMPLEMENTATION_START", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036D_PREMATURE_PROGRESSION",
        "IMP-036D start must keep implementation incomplete/unaccepted and IMP-036E unactivated",
      );
      break;
    }
  }
  ...
  if (
    !/IMP-036D\s*\|\s*Workforce & Franchise Operations Portal V2\s*\|\s*IMPLEMENTATION_IN_PROGRESS \/ AUTHORIZED \/ STARTED/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036D_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036D as IMPLEMENTATION_IN_PROGRESS / AUTHORIZED / STARTED",
    );
  }
  if (!/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036E_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036E Store Operations Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036D_D374_CREATED", "D-374 must not be created during IMP-036D implementation start");
  }
  if (!artifact) {
    fail("IMP036D_CAPABILITY_ARTIFACT", `Missing IMP-036D capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
  ...
  if (!experience) {
    ...
    const experienceRequired = [
      /Lifecycle:\s*IMPLEMENTATION_IN_PROGRESS/,
      /Architecture:\s*ARCHITECTURE_LOCKED/,
      /Implementation:\s*AUTHORIZED \/ STARTED/,
      /Founder UAT required:\s*YES/,
      /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/,
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036D_EXPERIENCE_START",
        "IMP-036D experience contract must record AUTHORIZED / STARTED with locked architecture facts",
      );
    }
    if (/IMP-036D_STARTED:\s*NO/.test(experienceText) || /Implementation:\s*AUTHORIZED \/ NOT_STARTED/.test(experienceText)) {
      fail("IMP036D_EXPERIENCE_STALE", "IMP-036D experience contract must not retain not-started implementation status");
    }
  }
  ...
  if (
    ...
    fail("IMP036D_IMP036C_REGRESSION", "IMP-036D start must not regress or reopen IMP-036C acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036D_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036D implementation start");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036D_DR_VERSION", "decision register must be DR-15 during IMP-036D implementation start");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? "")) {
    fail("IMP036D_ARCH_R20", "ARCH-R20 must not be created during IMP-036D implementation start");
  }

  const checkpoint = evaluateImp036dImplementationStartCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036c: /IMP-036C:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036d: /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentRoadmapSection) ? "IMPLEMENTATION_IN_PROGRESS" : "",
    architecture: /IMP-036D_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/.test(currentRoadmapSection)
      ? "AUTHORIZED / STARTED"
      : "",
    implementationAuthorized: /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    started: /IMP-036D_STARTED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036D_STARTED:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    implementationComplete: /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    accepted: /IMP-036D_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    imp036e: /IMP-036E:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_IN_PROGRESS"
      : "",
    stateLifecycle: /IMP-036D:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentStateAcceptance)
      ? "IMPLEMENTATION_IN_PROGRESS"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
  ...
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036D implementation started (${artifactRel})`);
}

function checkImp036dImplementationCompletion(roadmap, state, architecture, decision) {
  if (!isImp036dImplementationCompletionCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036dImplementationCompletionArtifact(artifactText);
  const artifactValid = artifact !== null && artifactValidation.ok;
  ...
  const requiredTokens = [
    [currentRoadmapSection, /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/, "ROADMAP must record IMP-036D IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"],
    [currentRoadmapSection, /IMP-036D_ARCHITECTURE:\s*LOCKED/, "ROADMAP must record IMP-036D architecture LOCKED"],
    [currentRoadmapSection, /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/, "ROADMAP must record IMP-036D architecture lock YES"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/, "ROADMAP must record IMP-036D AUTHORIZED / STARTED / COMPLETE"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/, "ROADMAP must record IMP-036D implementation authorized"],
    [currentRoadmapSection, /IMP-036D_STARTED:\s*YES/, "ROADMAP must record IMP-036D started"],
    [currentRoadmapSection, /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/, "ROADMAP must record IMP-036D complete"],
    [currentRoadmapSection, /IMP-036D_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036D unaccepted"],
    [currentRoadmapSection, /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036D Founder UAT required"],
    [currentRoadmapSection, /IMP-036D_FOUNDER_UAT:\s*NOT_STARTED/, "ROADMAP must record IMP-036D Founder UAT NOT_STARTED"],
    [currentRoadmapSection, /IMP036D_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/, "ROADMAP must record IMP-036D implementation evidence COMPLETE"],
    [currentRoadmapSection, /IMP_036D_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/, "ROADMAP must record IMP-036D independent implementation review PASS"],
    [currentRoadmapSection, /IMP036D_IMPLEMENTATION_MERGE_SHA:\s*b615fb20e034d71f49a1fa588f976d0592a46098/, "ROADMAP must record exact IMP-036D merge SHA"],
    [currentRoadmapSection, /IMP036D_IMPLEMENTATION_TREE:\s*8a5e75516ccee922e89c6202ad402e5a2acb24b9/, "ROADMAP must record exact IMP-036D merge tree"],
    [currentRoadmapSection, /Completion is \*\*not\*\* acceptance/, "ROADMAP must state completion is not acceptance"],
    [currentRoadmapSection, /IMP036D_PREPARATION_READINESS_DECISION:\s*NO_NEW_V1_DOMAIN_STATE_REQUIRED/, "ROADMAP must record preparation/readiness decision"],
    [currentRoadmapSection, /IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW:\s*DEFERRED/, "ROADMAP must defer FD workforce review"],
    [currentRoadmapSection, /IMP036D_NOTIFICATION_RESEND_WORKFORCE_TRANSPORT:\s*APPROVED_FOR_ARCHITECTURE/, "ROADMAP must record notification resend architecture direction"],
    [currentRoadmapSection, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "ROADMAP must record refund topology RESOLVED_AND_LOCKED"],
    ...
    [currentRoadmapSection, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED/, "ROADMAP must keep IMP-036E PLANNED / NOT_ACTIVATED"],
    [currentRoadmapSection, /IMP-036C:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036C COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/, "STATE must record IMP-036D IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"],
    [currentStateAcceptance, /IMP-036D_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036D architecture LOCKED"],
    [currentStateAcceptance, /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036D architecture lock YES"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/, "STATE must record IMP-036D AUTHORIZED / STARTED / COMPLETE"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/, "STATE must record IMP-036D implementation authorized"],
    [currentStateAcceptance, /IMP-036D_STARTED:\s*YES/, "STATE must record IMP-036D started"],
    [currentStateAcceptance, /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/, "STATE must record IMP-036D complete"],
    [currentStateAcceptance, /IMP-036D_ACCEPTED:\s*NO/, "STATE must record IMP-036D unaccepted"],
    [currentStateAcceptance, /IMP-036D_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036D Founder UAT required"],
    [currentStateAcceptance, /IMP-036D_FOUNDER_UAT:\s*NOT_STARTED/, "STATE must record IMP-036D Founder UAT NOT_STARTED"],
    [currentStateAcceptance, /IMP036D_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/, "STATE must record IMP-036D implementation evidence COMPLETE"],
    [currentStateAcceptance, /IMP_036D_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/, "STATE must record IMP-036D independent implementation review PASS"],
    [currentStateAcceptance, /IMP036D_IMPLEMENTATION_MERGE_SHA:\s*b615fb20e034d71f49a1fa588f976d0592a46098/, "STATE must record exact IMP-036D merge SHA"],
    [currentStateAcceptance, /IMP036D_IMPLEMENTATION_TREE:\s*8a5e75516ccee922e89c6202ad402e5a2acb24b9/, "STATE must record exact IMP-036D merge tree"],
    [currentStateAcceptance, /IMP036D_REVIEWED_CANDIDATE_HEAD:\s*72e321ffd90d4d6f009ee1457bb728b22fc0a2fd/, "STATE must record reviewed candidate head"],
    [currentStateAcceptance, /IMP036D_REVIEWED_CANDIDATE_TREE:\s*8a5e75516ccee922e89c6202ad402e5a2acb24b9/, "STATE must record reviewed candidate tree"],
    [currentStateAcceptance, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "STATE must record refund topology RESOLVED_AND_LOCKED"],
    [currentStateAcceptance, /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*NO/, "STATE must record refund topology no longer blocks lock"],
    [currentStateAcceptance, /D-374_CREATED:\s*NO/, "STATE must record D-374_CREATED: NO"],
    [currentStateAcceptance, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036E PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036C:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036C COMPLETE_AND_ACCEPTED"],
    [currentStateActivity, /IMP-036D IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/, "STATE current governance activity must record IMP-036D IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"],
  ...
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036D_IMPLEMENTATION_COMPLETION", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036D_PREMATURE_PROGRESSION",
        "IMP-036D completion must keep implementation unaccepted, Founder UAT not PASS, and IMP-036E unactivated",
      );
      break;
    }
  }
  ...
  if (
    !/IMP-036D\s*\|\s*Workforce & Franchise Operations Portal V2\s*\|\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036D_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036D as IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    );
  }
  if (!/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036E_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036E Store Operations Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036D_D374_CREATED", "D-374 must not be created during IMP-036D implementation completion");
  }
  if (!artifact) {
    fail("IMP036D_CAPABILITY_ARTIFACT", `Missing IMP-036D capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
  ...
  if (!experience) {
    ...
    const experienceRequired = [
      /Lifecycle:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/,
      /Architecture:\s*ARCHITECTURE_LOCKED/,
      /Implementation:\s*AUTHORIZED \/ STARTED \/ COMPLETE/,
      /Founder UAT required:\s*YES/,
      /Founder UAT:\s*NOT_STARTED/,
      /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/,
      /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/,
      /IMP-036D_STARTED:\s*YES/,
      /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/,
      /IMP-036D_ACCEPTED:\s*NO/,
      /pendingAcceptance\s*=\s*IMP-036D/,
      /IMP036D_PREPARATION_READINESS_DECISION\s*=\s*NO_NEW_V1_DOMAIN_STATE_REQUIRED/,
      /IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW\s*=\s*DEFERRED/,
      /IMP036D_NOTIFICATION_RESEND_WORKFORCE_TRANSPORT\s*=\s*APPROVED_FOR_ARCHITECTURE/,
      /IMP036D_REFUND_EXECUTION_TOPOLOGY\s*=\s*RESOLVED_AND_LOCKED/,
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036D_EXPERIENCE_COMPLETION",
        "IMP-036D experience contract must record IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE with locked architecture facts",
      );
    }
    if (
      /IMP-036D_IMPLEMENTATION_COMPLETE:\s*NO/.test(experienceText) ||
      /Lifecycle:\s*IMPLEMENTATION_IN_PROGRESS/.test(experienceText) ||
      /IMP-036D_FOUNDER_UAT:\s*PASS/.test(experienceText)
    ) {
      fail("IMP036D_EXPERIENCE_STALE", "IMP-036D experience contract must not retain incomplete or Founder-UAT-PASS status");
    }
  }
  ...
  if (
    state.meta.acceptedThrough !== "IMP-036C" ||
    state.meta.currentProductSlice !== "IMP-036D" ||
    state.meta.pendingAcceptance !== "IMP-036D" ||
    state.meta.nextProductSlice !== "IMP-036E"
  ) {
    fail(
      "IMP036D_STATE_POSITION",
      "STATE must record acceptedThrough IMP-036C, currentProductSlice IMP-036D, nextProductSlice IMP-036E, pendingAcceptance IMP-036D",
    );
  }
  if (
    !/IMP-036C:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) ||
    ...
    fail("IMP036D_IMP036C_REGRESSION", "IMP-036D completion must not regress or reopen IMP-036C acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036D_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036D implementation completion");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036D_DR_VERSION", "decision register must be DR-15 during IMP-036D implementation completion");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? "")) {
    fail("IMP036D_ARCH_R20", "ARCH-R20 must not be created during IMP-036D implementation completion");
  }

  const checkpoint = evaluateImp036dImplementationCompletionCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036c: /IMP-036C:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036d: /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    architecture: /IMP-036D_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036D_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/.test(currentRoadmapSection)
      ? "AUTHORIZED / STARTED / COMPLETE"
      : "",
    implementationAuthorized: /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentStateAcceptance)
    ...
    started: /IMP-036D_STARTED:\s*YES/.test(currentRoadmapSection) &&
      ...
      ? "YES"
      : "",
    implementationComplete: /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    accepted: /IMP-036D_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    imp036e: /IMP-036E:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    stateLifecycle: /IMP-036D:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentStateAcceptance)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
  ...
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036D implementation complete pending acceptance (${artifactRel})`);
}

function checkImp036dAcceptance(roadmap, state, architecture, decision) {
  if (!isImp036dAcceptanceCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036dAcceptanceArtifact(artifactText);
  if (artifact !== null && !artifactValidation.ok) fail(artifactValidation.code, artifactValidation.message);

  if (/IMP-036D\s*\|\s*Workforce & Franchise Operations Portal V2\s*\|\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(futureSection)) {
    fail("IMP036D_ROADMAP_FUTURE", "ROADMAP future ledger must not retain IMP-036D as pending acceptance");
  }
  if (!/IMP-036D\s*\|\s*Workforce & Franchise Operations Portal V2\s*\|\s*COMPLETE_AND_ACCEPTED/.test(acceptedSection)) {
    fail("IMP036D_ROADMAP_LIFECYCLE", "ROADMAP accepted ledger must list IMP-036D as COMPLETE_AND_ACCEPTED");
  }
  if (!/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036E_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036E PLANNED");
  }

  const requiredTokens = [
    [currentRoadmapSection, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must record IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentRoadmapSection, /IMP-036D_ACCEPTED:\s*YES/, "ROADMAP must record IMP-036D accepted"],
    [currentRoadmapSection, /IMP036D_FORMAL_ACCEPTANCE:\s*ACCEPTED/, "ROADMAP must record IMP-036D formal acceptance ACCEPTED"],
    [currentRoadmapSection, /IMP036D_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/, "ROADMAP must record IMP-036D independent acceptance evidence ACCEPTED"],
    [currentRoadmapSection, /IMP-036D_FOUNDER_UAT:\s*PASS/, "ROADMAP must record IMP-036D Founder UAT PASS"],
    [currentRoadmapSection, /IMP036D_ACCEPTED_MAIN_SHA:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/, "ROADMAP must record accepted product SHA"],
    [currentRoadmapSection, /IMP036D_ACCEPTED_TREE:\s*6580497091525c3ddd892aa44aafd63ef1132d35/, "ROADMAP must record accepted product tree"],
    [currentRoadmapSection, /IMP036D_ACCEPTED_CANDIDATE:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/, "ROADMAP must record accepted candidate"],
    [currentRoadmapSection, /IMP036D_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*3d999b0a154e70b9332d8f5228d1723ddd501cf8b6ec162ebec8c1bfc042a735/, "ROADMAP must record Founder UAT fingerprint"],
    [currentRoadmapSection, /IMP036D_IMPLEMENTATION_MERGE_SHA:\s*b615fb20e034d71f49a1fa588f976d0592a46098/, "ROADMAP must preserve implementation merge SHA"],
    [currentRoadmapSection, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "ROADMAP must preserve refund topology RESOLVED_AND_LOCKED"],
    [currentRoadmapSection, /IMP036D_REFUND_TOPOLOGY_BLOCKS_ARCHITECTURE_LOCK:\s*NO/, "ROADMAP must preserve refund topology unlock"],
    [currentRoadmapSection, /IMP036D_REFUND_MUTATION_TRANSPORT_LOCKED:\s*YES/, "ROADMAP must preserve refund mutation transport locked"],
    [currentRoadmapSection, /IMP036D_PREPARATION_READINESS_DECISION:\s*NO_NEW_V1_DOMAIN_STATE_REQUIRED/, "ROADMAP must preserve preparation readiness decision"],
    [currentRoadmapSection, /IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW:\s*DEFERRED/, "ROADMAP must preserve FD workforce review deferred"],
    [currentRoadmapSection, /SCHEMA_CHANGE_REQUIRED:\s*NO/, "ROADMAP must record SCHEMA_CHANGE_REQUIRED: NO"],
    [currentRoadmapSection, /D-374_CREATED:\s*NO/, "ROADMAP must record D-374_CREATED: NO"],
    [currentRoadmapSection, /ARCH_R20_REQUIRED:\s*NO/, "ROADMAP must record ARCH_R20_REQUIRED: NO"],
    [currentRoadmapSection, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "ROADMAP must keep IMP-036E planned"],
    [currentRoadmapSection, /IMP-036C:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036C COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "STATE must record IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036D_ACCEPTED:\s*YES/, "STATE must record IMP-036D accepted"],
    [currentStateAcceptance, /IMP036D_FORMAL_ACCEPTANCE:\s*ACCEPTED/, "STATE must record IMP-036D formal acceptance ACCEPTED"],
    [currentStateAcceptance, /IMP036D_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/, "STATE must record IMP-036D independent acceptance evidence ACCEPTED"],
    [currentStateAcceptance, /IMP-036D_FOUNDER_UAT:\s*PASS/, "STATE must record IMP-036D Founder UAT PASS"],
    [currentStateAcceptance, /IMP036D_ACCEPTED_MAIN_SHA:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/, "STATE must record accepted product SHA"],
    [currentStateAcceptance, /IMP036D_ACCEPTED_TREE:\s*6580497091525c3ddd892aa44aafd63ef1132d35/, "STATE must record accepted product tree"],
    [currentStateAcceptance, /IMP036D_ACCEPTED_CANDIDATE:\s*a6ff612c65e0d58409017b2935e0da16cffa9530/, "STATE must record accepted candidate"],
    [currentStateAcceptance, /IMP036D_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*3d999b0a154e70b9332d8f5228d1723ddd501cf8b6ec162ebec8c1bfc042a735/, "STATE must record Founder UAT fingerprint"],
    [currentStateAcceptance, /IMP036D_IMPLEMENTATION_MERGE_SHA:\s*b615fb20e034d71f49a1fa588f976d0592a46098/, "STATE must preserve implementation merge SHA"],
    [currentStateAcceptance, /IMP036D_REFUND_EXECUTION_TOPOLOGY:\s*RESOLVED_AND_LOCKED/, "STATE must preserve refund topology RESOLVED_AND_LOCKED"],
    [currentStateAcceptance, /D-374_CREATED:\s*NO/, "STATE must record D-374_CREATED: NO"],
    [currentStateAcceptance, /IMP-036E:\s*PLANNED \/ NOT_ACTIVATED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "STATE must keep IMP-036E planned"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036D_ACCEPTANCE", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail("IMP036D_PREMATURE_PROGRESSION", "IMP-036D acceptance must not activate, authorize, or start IMP-036E");
      break;
    }
  }

  if (
    state.meta.acceptedThrough !== "IMP-036D" ||
    state.meta.currentProductSlice !== "NONE" ||
    state.meta.pendingAcceptance !== "NONE" ||
    state.meta.nextProductSlice !== "IMP-036E"
  ) {
    fail("IMP036D_STATE_POSITION", "STATE must record acceptedThrough IMP-036D, currentProductSlice NONE, nextProductSlice IMP-036E, pendingAcceptance NONE");
  }
  if (!/IMP-036D COMPLETE_AND_ACCEPTED/.test(currentStateActivity) || !/IMP-036E/.test(currentStateActivity)) {
    fail("IMP036D_STATE_ACTIVITY", "STATE current governance activity must record IMP-036D COMPLETE_AND_ACCEPTED and IMP-036E planned");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036D_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036D acceptance");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036D_DR_VERSION", "decision register must be DR-15 during IMP-036D acceptance");
  }
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036D_D374_CREATED", "D-374 must not be created during IMP-036D acceptance");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? "")) {
    fail("IMP036D_ARCH_R20", "ARCH-R20 must not be created during IMP-036D acceptance");
  }

  const checkpoint = evaluateImp036dAcceptanceCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036c: /IMP-036C:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036d: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    architecture: /IMP-036D_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036D_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementationAuthorized: /IMP-036D_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    started: /IMP-036D_STARTED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementationComplete: /IMP-036D_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    accepted: /IMP-036D_ACCEPTED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
    artifact: artifact !== null && artifactValidation.ok,
    founderUatPass: /IMP-036D_FOUNDER_UAT:\s*PASS/.test(currentRoadmapSection),
    schemaChangeRequired: /SCHEMA_CHANGE_REQUIRED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    d374Exists: /\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? ""),
    archR20Exists: /architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? ""),
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036D COMPLETE_AND_ACCEPTED (${artifactRel})`);
}


function checkImp036eAcceptance(roadmap, state, architecture, decision) {
  if (!isImp036eAcceptanceCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036eAcceptanceArtifact(artifactText);
  if (artifact !== null && !artifactValidation.ok) fail(artifactValidation.code, artifactValidation.message);

  if (/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(futureSection)) {
    fail("IMP036E_ROADMAP_FUTURE", "ROADMAP future ledger must not retain IMP-036E as pending acceptance");
  }
  if (!/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*COMPLETE_AND_ACCEPTED/.test(acceptedSection)) {
    fail("IMP036E_ROADMAP_LIFECYCLE", "ROADMAP accepted ledger must list IMP-036E as COMPLETE_AND_ACCEPTED");
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F PLANNED");
  }

  const requiredTokens = [
    [currentRoadmapSection, /IMP-036E:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must record IMP-036E COMPLETE_AND_ACCEPTED"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*YES/, "ROADMAP must record IMP-036E accepted"],
    [currentRoadmapSection, /IMP036E_FORMAL_ACCEPTANCE:\s*ACCEPTED/, "ROADMAP must record IMP-036E formal acceptance ACCEPTED"],
    [currentRoadmapSection, /IMP036E_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/, "ROADMAP must record IMP-036E independent acceptance evidence ACCEPTED"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT:\s*PASS/, "ROADMAP must record IMP-036E Founder UAT PASS"],
    [currentRoadmapSection, /IMP036E_ACCEPTED_MAIN_SHA:\s*05c534bac3d077f5ab89928495568bb63faf78df/, "ROADMAP must record accepted product SHA"],
    [currentRoadmapSection, /IMP036E_ACCEPTED_TREE:\s*55b28977ee9860c2c07cb25f751c9f48ef4a2aa6/, "ROADMAP must record accepted product tree"],
    [currentRoadmapSection, /IMP036E_ACCEPTED_CANDIDATE:\s*05c534bac3d077f5ab89928495568bb63faf78df/, "ROADMAP must record accepted candidate"],
    [currentRoadmapSection, /IMP036E_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*1a97d3a4c80394804e19398e4f3684067aefb0d42a01a7899e8777d1a07cb289/, "ROADMAP must record Founder UAT fingerprint"],
    [currentRoadmapSection, /IMP036E_FOUNDER_UAT_DECISION_DATE:\s*2026-09-10/, "ROADMAP must record Founder UAT decision date"],
    [currentRoadmapSection, /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/, "ROADMAP must preserve implementation merge SHA"],
    [currentRoadmapSection, /IMP036E_REVIEWED_CANDIDATE_HEAD:\s*b2ffaa5bc2b5c6f58ff5241c226b583160132837/, "ROADMAP must preserve reviewed candidate HEAD"],
    [currentRoadmapSection, /FOUNDER_STAGING_INTERMEDIATE_CANDIDATE_SHA:\s*e9821271a29ae35ba6c921008b976cd2e8d15c50/, "ROADMAP must preserve intermediate staging candidate SHA"],
    ...
    [currentRoadmapSection, /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY:\s*NO/, "ROADMAP must record postal/PIN runtime authority NO"],
    [currentRoadmapSection, /SCHEMA_CHANGE_REQUIRED:\s*NO/, "ROADMAP must record SCHEMA_CHANGE_REQUIRED: NO"],
    [currentRoadmapSection, /D-374_CREATED:\s*NO/, "ROADMAP must record D-374_CREATED: NO"],
    [currentRoadmapSection, /ARCH_R20_CREATED:\s*NO/, "ROADMAP must record ARCH_R20_CREATED: NO"],
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "ROADMAP must keep IMP-036F planned"],
    [currentRoadmapSection, /IMP036F_ACTIVATED:\s*NO/, "ROADMAP must record IMP036F_ACTIVATED: NO"],
    [currentRoadmapSection, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036E:\s*COMPLETE_AND_ACCEPTED/, "STATE must record IMP-036E COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*YES/, "STATE must record IMP-036E accepted"],
    [currentStateAcceptance, /IMP036E_FORMAL_ACCEPTANCE:\s*ACCEPTED/, "STATE must record IMP-036E formal acceptance ACCEPTED"],
    [currentStateAcceptance, /IMP036E_INDEPENDENT_ACCEPTANCE_EVIDENCE:\s*ACCEPTED/, "STATE must record IMP-036E independent acceptance evidence ACCEPTED"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT:\s*PASS/, "STATE must record IMP-036E Founder UAT PASS"],
    [currentStateAcceptance, /IMP036E_ACCEPTED_MAIN_SHA:\s*05c534bac3d077f5ab89928495568bb63faf78df/, "STATE must record accepted product SHA"],
    [currentStateAcceptance, /IMP036E_ACCEPTED_TREE:\s*55b28977ee9860c2c07cb25f751c9f48ef4a2aa6/, "STATE must record accepted product tree"],
    [currentStateAcceptance, /IMP036E_ACCEPTED_CANDIDATE:\s*05c534bac3d077f5ab89928495568bb63faf78df/, "STATE must record accepted candidate"],
    [currentStateAcceptance, /IMP036E_FOUNDER_UAT_CANDIDATE_FINGERPRINT:\s*1a97d3a4c80394804e19398e4f3684067aefb0d42a01a7899e8777d1a07cb289/, "STATE must record Founder UAT fingerprint"],
    [currentStateAcceptance, /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/, "STATE must preserve implementation merge SHA"],
    [currentStateAcceptance, /D-374_CREATED:\s*NO/, "STATE must record D-374_CREATED: NO"],
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "STATE must keep IMP-036F planned"],
    [currentStateAcceptance, /IMP036F_ACTIVATED:\s*NO/, "STATE must record IMP036F_ACTIVATED: NO"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036E_ACCEPTANCE", message);
  }

  const premature = [
    /IMP-036F_IMPLEMENTATION_AUTHORIZED:\s*YES/,
    /IMP-036F_STARTED:\s*YES/,
    /IMP-036F:\s*ARCHITECTURE_IN_PROGRESS/,
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail("IMP036E_PREMATURE_PROGRESSION", "IMP-036E acceptance must not activate, authorize, or start IMP-036F");
      break;
    }
  }
  ...
  if (
    ...
    state.meta.currentProductSlice !== "NONE" ||
    state.meta.pendingAcceptance !== "NONE" ||
    state.meta.nextProductSlice !== "IMP-036F"
  ) {
    fail("IMP036E_STATE_POSITION", "STATE must record acceptedThrough IMP-036E, currentProductSlice NONE, nextProductSlice IMP-036F, pendingAcceptance NONE");
  }
  if (!/IMP-036E COMPLETE_AND_ACCEPTED/.test(currentStateActivity) || !/IMP-036F/.test(currentStateActivity)) {
    fail("IMP036E_STATE_ACTIVITY", "STATE current governance activity must record IMP-036E COMPLETE_AND_ACCEPTED and IMP-036F planned");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E acceptance");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E acceptance");
  }
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E acceptance");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20") {
    fail("IMP036E_ARCH_R20", "ARCH-R20 must not be created during IMP-036E acceptance");
  }

  const checkpoint = evaluateImp036eAcceptanceCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    architecture: /IMP-036E_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementationAuthorized: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    started: /IMP-036E_STARTED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementationComplete: /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    accepted: /IMP-036E_ACCEPTED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
    artifact: artifact !== null && artifactValidation.ok,
    founderUatPass: /IMP-036E_FOUNDER_UAT:\s*PASS/.test(currentRoadmapSection),
    schemaChangeRequired: /SCHEMA_CHANGE_REQUIRED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    d374Exists: /\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? ""),
    archR20Exists: /architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20",
    imp036fActivated: /IMP036F_ACTIVATED:\s*YES/.test(`${currentRoadmapSection}\n${currentStateAcceptance}`),
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036E COMPLETE_AND_ACCEPTED (${artifactRel})`);
}


function checkImp036eArchitectureLock(roadmap, state, architecture, decision) {
  if (!isImp036eArchitectureLockCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036eArchitectureLockArtifact(artifactText);
  const artifactValid = artifact !== null && artifactValidation.ok;
  ...
  const requiredTokens = [
    [currentRoadmapSection, /IMP-036E:\s*ARCHITECTURE_LOCKED/, "ROADMAP must record IMP-036E architecture locked"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "ROADMAP must record IMP-036E architecture LOCKED"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "ROADMAP must record IMP-036E architecture lock YES"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/, "ROADMAP must record IMP-036E implementation not authorized"],
    [currentRoadmapSection, /IMP-036E_STARTED:\s*NO/, "ROADMAP must record IMP-036E not started"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "ROADMAP must record IMP-036E incomplete"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036E unaccepted"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036E Founder UAT required"],
    ...
    [currentRoadmapSection, /NEW_SCOPE_MODEL:\s*NO/, "ROADMAP must record NEW_SCOPE_MODEL: NO"],
    [currentRoadmapSection, /D374_REQUIRED_FOR_IMP036E_LOCK:\s*NO/, "ROADMAP must record D374 not required"],
    [currentRoadmapSection, /D-374_CREATED:\s*NO/, "ROADMAP must record D-374_CREATED: NO"],
    [currentRoadmapSection, /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK:\s*NO/, "ROADMAP must record ARCH-R20 not required"],
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "ROADMAP must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentRoadmapSection, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036E:\s*ARCHITECTURE_LOCKED/, "STATE must record IMP-036E architecture locked"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036E architecture LOCKED"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036E architecture lock YES"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/, "STATE must record IMP-036E implementation not authorized"],
    [currentStateAcceptance, /IMP-036E_STARTED:\s*NO/, "STATE must record IMP-036E not started"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "STATE must record IMP-036E incomplete"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*NO/, "STATE must record IMP-036E unaccepted"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036E Founder UAT required"],
    ...
    [currentStateAcceptance, /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY:\s*NO/, "STATE must record postal/PIN runtime authority NO"],
    ...
    [currentStateAcceptance, /NEW_SCOPE_MODEL:\s*NO/, "STATE must record NEW_SCOPE_MODEL: NO"],
    [currentStateAcceptance, /D374_REQUIRED_FOR_IMP036E_LOCK:\s*NO/, "STATE must record D374 not required"],
    [currentStateAcceptance, /D-374_CREATED:\s*NO/, "STATE must record D-374_CREATED: NO"],
    [currentStateAcceptance, /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK:\s*NO/, "STATE must record ARCH-R20 not required"],
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_PREMATURE_PROGRESSION",
        "IMP-036E lock must not authorize/start implementation, accept the slice, or activate IMP-036F",
      );
      break;
    }
  }
  ...
  if (
    !/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*ARCHITECTURE_LOCKED/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036E_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036E Store Operations Management as ARCHITECTURE_LOCKED",
    );
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F Catalog, Menu, Pricing & Promotions Management PLANNED");
  }
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E architecture lock");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20") {
    fail("IMP036E_ARCH_R20_CREATED", "ARCH-R20 must not be created during IMP-036E architecture lock");
  }
  if (!artifact) {
    fail("IMP036E_CAPABILITY_ARTIFACT", `Missing IMP-036E capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
    fail(artifactValidation.code, artifactValidation.message);
  }
  if (!experience) {
    fail("IMP036E_EXPERIENCE_MISSING", `Missing IMP-036E experience plan at ${experienceRel}`);
  } else {
    const experienceRequired = [
      /Status:\s*ARCHITECTURE_LOCKED SUPPORTING EXPERIENCE CONTRACT/,
      /Lifecycle:\s*ARCHITECTURE_LOCKED/,
      /Architecture:\s*LOCKED/,
      /Implementation:\s*NOT_AUTHORIZED \/ NOT_STARTED/,
      /Founder UAT required:\s*YES/,
      /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/,
      /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/,
      ...
      /IMP036E_SERVICEABILITY_ROUTING_PRIORITY_UI\s*=\s*HIDDEN_PREREQUISITE/,
      /IMP036E_SESSION_CAPABILITY_PROJECTION_EXTENSION\s*=\s*EXISTING_PERMISSION_KEYS_ONLY/,
      /SCHEMA_CHANGE_REQUIRED\s*=\s*NO/,
      /D374_REQUIRED_FOR_IMP036E_LOCK\s*=\s*NO/,
      /D-374_CREATED:\s*NO/,
      /ARCH_R20_REQUIRED_FOR_IMP036E_LOCK\s*=\s*NO/,
      /capabilities\/IMP-036E-store-operations-management\.md/,
    ];
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036E_EXPERIENCE_LOCK",
        "IMP-036E experience contract must record architecture-locked decisions and Founder-A markers",
      );
    }
  }
  ...
  if (
    state.meta.acceptedThrough !== "IMP-036D" ||
    state.meta.currentProductSlice !== "IMP-036E" ||
    state.meta.pendingAcceptance !== "NONE" ||
    state.meta.nextProductSlice !== "IMP-036F"
  ) {
    fail(
      "IMP036E_STATE_POSITION",
      "STATE must record acceptedThrough IMP-036D, currentProductSlice IMP-036E, nextProductSlice IMP-036F, pendingAcceptance NONE",
    );
  }
  if (!/IMP-036E ARCHITECTURE_LOCKED/.test(currentStateActivity)) {
    fail("IMP036E_STATE_ACTIVITY", "STATE current governance activity must record IMP-036E ARCHITECTURE_LOCKED");
  }
  if (
    !/IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) ||
    /IMP-036D_ACCEPTED:\s*NO/.test(currentStateAcceptance)
  ) {
    fail("IMP036E_IMP036D_REGRESSION", "IMP-036E lock must not regress or reopen IMP-036D acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E architecture lock");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E architecture lock");
  }

  const checkpoint = evaluateImp036eArchitectureLockCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036d: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036e: /IMP-036E:\s*ARCHITECTURE_LOCKED/.test(currentRoadmapSection) ? "ARCHITECTURE_LOCKED" : "",
    architecture: /IMP-036E_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) &&
      /IMP-036E_STARTED:\s*NO/.test(currentRoadmapSection)
      ? "NOT_AUTHORIZED / NOT_STARTED"
      : "",
    implementationAuthorized: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    started: /IMP-036E_STARTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    implementationComplete: /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    accepted: /IMP-036E_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    founderUatRequired: /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    imp036f: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036E:\s*ARCHITECTURE_LOCKED/.test(currentRoadmapSection)
      ? "ARCHITECTURE_LOCKED"
      : "",
    stateLifecycle: /IMP-036E:\s*ARCHITECTURE_LOCKED/.test(currentStateAcceptance)
      ? "ARCHITECTURE_LOCKED"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
    schemaChangeRequired: /SCHEMA_CHANGE_REQUIRED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    d374Exists: /\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? ""),
    archR20Exists: architecture?.meta?.architectureVersion === "ARCH-R20" ||
      /"architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? ""),
    capabilityArtifactExists: Boolean(artifact),
    artifactValid,
    imp036dAccepted: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) &&
      /IMP-036D_ACCEPTED:\s*YES/.test(currentStateAcceptance),
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036E capability architecture locked (${artifactRel})`);
}

function checkImp036eImplementationAuthorization(roadmap, state, architecture, decision) {
  if (!isImp036eImplementationAuthorizationCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036eImplementationAuthorizationArtifact(artifactText);
  const artifactValid = artifact !== null && artifactValidation.ok;
  ...
  const requiredTokens = [
    ...
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "ROADMAP must record IMP-036E architecture LOCKED"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "ROADMAP must record IMP-036E architecture lock YES"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ NOT_STARTED/, "ROADMAP must record IMP-036E AUTHORIZED / NOT_STARTED"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "ROADMAP must record IMP-036E implementation authorized"],
    [currentRoadmapSection, /IMP-036E_STARTED:\s*NO/, "ROADMAP must record IMP-036E not started"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "ROADMAP must record IMP-036E incomplete"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036E unaccepted"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036E Founder UAT required"],
    ...
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036E architecture LOCKED"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036E architecture lock YES"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ NOT_STARTED/, "STATE must record IMP-036E AUTHORIZED / NOT_STARTED"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "STATE must record IMP-036E implementation authorized"],
    [currentStateAcceptance, /IMP-036E_STARTED:\s*NO/, "STATE must record IMP-036E not started"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "STATE must record IMP-036E incomplete"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*NO/, "STATE must record IMP-036E unaccepted"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036E Founder UAT required"],
    ...
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateActivity, /IMP-036E ARCHITECTURE_LOCKED/, "STATE current governance activity must record IMP-036E ARCHITECTURE_LOCKED"],
    [currentStateActivity, /AUTHORIZED \/ NOT_STARTED/, "STATE current governance activity must record AUTHORIZED / NOT_STARTED"],
    [currentStateActivity, /Current Product Implementation:\s*NONE/, "STATE must keep Current Product Implementation NONE"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036E_IMPLEMENTATION_AUTHORIZATION", message);
  }
  ...
  const correctiveForbidden = [
    ...
    /Outlet-scoped permission union grants Brand Assortment authority/i,
    /global capability booleans are sufficient authorization for selected Store/i,
    /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY:\s*YES/,
  ];
  for (const text of [currentRoadmapSection, currentStateAcceptance, artifactText]) {
    if (correctiveForbidden.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_CORRECTIVE_SEMANTICS",
        "IMP-036E authorization must keep Assortment Store transport read-only, global session caps non-authoritative, and PIN non-authoritative",
      );
      break;
    }
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_PREMATURE_PROGRESSION",
        "IMP-036E authorization must keep implementation unstarted and IMP-036F unactivated",
      );
      break;
    }
  }
  if (
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) ||
    /IMP-036E_IMPLEMENTATION:\s*NOT_AUTHORIZED \/ NOT_STARTED/.test(currentRoadmapSection)
  ) {
    fail(
      "IMP036E_STALE_UNAUTHORIZED",
      "IMP-036E authorization checkpoint must not retain NOT_AUTHORIZED / NOT_STARTED status",
    );
  }

  if (
    !/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*ARCHITECTURE_LOCKED \/ AUTHORIZED \/ NOT_STARTED/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036E_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036E as ARCHITECTURE_LOCKED / AUTHORIZED / NOT_STARTED",
    );
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F Catalog, Menu, Pricing & Promotions Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E implementation authorization");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20") {
    fail("IMP036E_ARCH_R20_CREATED", "ARCH-R20 must not be created during IMP-036E implementation authorization");
  }
  if (!artifact) {
    fail("IMP036E_CAPABILITY_ARTIFACT", `Missing IMP-036E capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
  ...
  if (!experience) {
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036E_EXPERIENCE_AUTHORIZATION",
        "IMP-036E experience contract must record AUTHORIZED / NOT_STARTED with locked architecture facts",
      );
    }
    if (/IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(experienceText)) {
      fail("IMP036E_EXPERIENCE_STALE", "IMP-036E experience contract must not retain unauthorized implementation status");
    }
  }
  ...
  if (
    ...
    fail("IMP036E_IMP036D_REGRESSION", "IMP-036E authorization must not regress or reopen IMP-036D acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E implementation authorization");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E implementation authorization");
  }

  const checkpoint = evaluateImp036eImplementationAuthorizationCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    architecture: /IMP-036E_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ NOT_STARTED/.test(currentRoadmapSection)
      ? "AUTHORIZED / NOT_STARTED"
      : "",
    implementationAuthorized: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    started: /IMP-036E_STARTED:\s*NO/.test(currentRoadmapSection) &&
      /IMP-036E_STARTED:\s*NO/.test(currentStateAcceptance)
      ? "NO"
      : "",
    implementationComplete: /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    accepted: /IMP-036E_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    ...
    archR20Exists: architecture?.meta?.architectureVersion === "ARCH-R20" ||
      /"architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? ""),
    capabilityArtifactExists: Boolean(artifact),
    capabilityArtifactValid: artifactValid,
    artifactValid,
    imp036dAccepted: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) &&
      /IMP-036D_ACCEPTED:\s*YES/.test(currentStateAcceptance),
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036E implementation authorized / not started (${artifactRel})`);
}

function checkImp036eImplementationStart(roadmap, state, architecture, decision) {
  if (!isImp036eImplementationStartCheckpoint(roadmap, state)) return;
  ...
  const artifact = resolveExactRelativeFile(artifactRel);
  const artifactText = artifact ? readFileSync(artifact, "utf8") : "";
  const artifactValidation = evaluateImp036eImplementationStartArtifact(artifactText);
  const artifactValid = artifact !== null && artifactValidation.ok;
  ...
  const requiredTokens = [
    [currentRoadmapSection, /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/, "ROADMAP must record IMP-036E IMPLEMENTATION_IN_PROGRESS"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "ROADMAP must record IMP-036E architecture LOCKED"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "ROADMAP must record IMP-036E architecture lock YES"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/, "ROADMAP must record IMP-036E AUTHORIZED / STARTED"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "ROADMAP must record IMP-036E implementation authorized"],
    [currentRoadmapSection, /IMP-036E_STARTED:\s*YES/, "ROADMAP must record IMP-036E started"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "ROADMAP must record IMP-036E incomplete"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036E unaccepted"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036E Founder UAT required"],
    [currentRoadmapSection, /Start does \*\*not\*\* complete or\s+accept implementation/, "ROADMAP must state start does not complete or accept"],
    ...
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "ROADMAP must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentRoadmapSection, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/, "STATE must record IMP-036E IMPLEMENTATION_IN_PROGRESS"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036E architecture LOCKED"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036E architecture lock YES"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/, "STATE must record IMP-036E AUTHORIZED / STARTED"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "STATE must record IMP-036E implementation authorized"],
    [currentStateAcceptance, /IMP-036E_STARTED:\s*YES/, "STATE must record IMP-036E started"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "STATE must record IMP-036E incomplete"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*NO/, "STATE must record IMP-036E unaccepted"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036E Founder UAT required"],
    [currentStateAcceptance, /IMP036E_ASSORTMENT_AUTHORITY:\s*BRAND/, "STATE must record Founder-A Assortment BRAND authority"],
    ...
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateActivity, /IMP-036E IMPLEMENTATION_IN_PROGRESS/, "STATE current governance activity must record IMP-036E IMPLEMENTATION_IN_PROGRESS"],
    [currentStateActivity, /AUTHORIZED \/ STARTED/, "STATE current governance activity must record AUTHORIZED / STARTED"],
    [currentStateActivity, /Current Product Implementation:\s*IMP-036E/, "STATE must record Current Product Implementation IMP-036E"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036E_IMPLEMENTATION_START", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, artifactText]) {
    if (correctiveForbidden.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_CORRECTIVE_SEMANTICS",
        "IMP-036E start must keep Assortment Store transport read-only, global session caps non-authoritative, and PIN non-authoritative",
      );
      break;
    }
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_PREMATURE_PROGRESSION",
        "IMP-036E start must keep implementation incomplete/unaccepted and IMP-036F unactivated",
      );
      break;
    }
  }
  if (
    /IMP-036E_STARTED:\s*NO/.test(currentRoadmapSection) ||
    /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) ||
    /Current Product Implementation:\s*NONE/.test(currentStateActivity.split("Current Governance Activity")[0] ?? "")
  ) {
    fail(
      "IMP036E_STALE_NOT_STARTED",
      "IMP-036E start checkpoint must record STARTED YES, AUTHORIZED YES, and Current Product Implementation IMP-036E",
    );
  }

  if (
    !/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*IMPLEMENTATION_IN_PROGRESS \/ AUTHORIZED \/ STARTED/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036E_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036E as IMPLEMENTATION_IN_PROGRESS / AUTHORIZED / STARTED",
    );
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F Catalog, Menu, Pricing & Promotions Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E implementation start");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20") {
    fail("IMP036E_ARCH_R20_CREATED", "ARCH-R20 must not be created during IMP-036E implementation start");
  }
  if (!artifact) {
    fail("IMP036E_CAPABILITY_ARTIFACT", `Missing IMP-036E capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
  ...
  if (!experience) {
    ...
    const experienceRequired = [
      ...
      /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
      /Founder implementation-start authorization recorded at\s*GTM-R112 \/ STATE-R110/,
      /This start gate contains no product implementation/,
      /IMP036E_ASSORTMENT_AUTHORITY\s*=\s*BRAND/,
      /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE\s*=\s*NO/,
      /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION\s*=\s*AUTHORIZED_READ_OR_ESCALATE/,
      /IMP036E_ASSORTMENT_WORKFORCE_TRANSPORT\s*=\s*READ_ONLY_OPERATIONS_PROJECTION/,
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036E_EXPERIENCE_START",
        "IMP-036E experience contract must record AUTHORIZED / STARTED with locked architecture facts",
      );
    }
    if (
      /IMP-036E_STARTED:\s*NO/.test(experienceText) ||
      /Implementation:\s*AUTHORIZED \/ NOT_STARTED/.test(experienceText) ||
      /Lifecycle:\s*ARCHITECTURE_LOCKED/.test(experienceText)
    ) {
      fail("IMP036E_EXPERIENCE_STALE", "IMP-036E experience contract must not retain not-started or architecture-locked-only lifecycle");
    }
  }
  ...
  if (
    ...
    fail("IMP036E_IMP036D_REGRESSION", "IMP-036E start must not regress or reopen IMP-036D acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E implementation start");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E implementation start");
  }

  const checkpoint = evaluateImp036eImplementationStartCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    nextProductSlice: state.meta.nextProductSlice,
    pendingAcceptance: state.meta.pendingAcceptance,
    currentProductImplementation: /Current Product Implementation:\s*IMP-036E/.test(currentStateActivity)
      ? "IMP-036E"
      : "",
    imp036d: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036e: /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentRoadmapSection) ? "IMPLEMENTATION_IN_PROGRESS" : "",
    architecture: /IMP-036E_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED/.test(currentRoadmapSection)
      ? "AUTHORIZED / STARTED"
      : "",
    implementationAuthorized: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    started: /IMP-036E_STARTED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036E_STARTED:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    implementationComplete: /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    accepted: /IMP-036E_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    founderUatRequired: /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    imp036f: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_IN_PROGRESS"
      : "",
    stateLifecycle: /IMP-036E:\s*IMPLEMENTATION_IN_PROGRESS/.test(currentStateAcceptance)
      ? "IMPLEMENTATION_IN_PROGRESS"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
  ...
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036E implementation started (${artifactRel})`);
}

function checkImp036eImplementationCompletion(roadmap, state, architecture, decision) {
  if (!isImp036eImplementationCompletionCheckpoint(roadmap, state)) return;
  ...
  const requiredTokens = [
    ...
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "ROADMAP must record IMP-036E implementation authorized"],
    [currentRoadmapSection, /IMP-036E_STARTED:\s*YES/, "ROADMAP must record IMP-036E started"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/, "ROADMAP must record IMP-036E complete"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036E unaccepted"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "ROADMAP must record IMP-036E Founder UAT required"],
    [currentRoadmapSection, /IMP-036E_FOUNDER_UAT:\s*NOT_STARTED/, "ROADMAP must record IMP-036E Founder UAT NOT_STARTED"],
    [currentRoadmapSection, /IMP036E_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/, "ROADMAP must record IMP-036E implementation evidence COMPLETE"],
    [currentRoadmapSection, /IMP_036E_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/, "ROADMAP must record IMP-036E independent implementation review PASS"],
    [currentRoadmapSection, /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/, "ROADMAP must record exact IMP-036E merge SHA"],
    [currentRoadmapSection, /IMP036E_IMPLEMENTATION_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/, "ROADMAP must record exact IMP-036E merge tree"],
    [currentRoadmapSection, /IMP036E_REVIEWED_CANDIDATE_HEAD:\s*b2ffaa5bc2b5c6f58ff5241c226b583160132837/, "ROADMAP must record reviewed candidate head"],
    [currentRoadmapSection, /IMP036E_REVIEWED_CANDIDATE_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/, "ROADMAP must record reviewed candidate tree"],
    [currentRoadmapSection, /Completion is \*\*not\*\* acceptance/, "ROADMAP must state completion is not acceptance"],
    [currentRoadmapSection, /IMP036E_ASSORTMENT_AUTHORITY:\s*BRAND/, "ROADMAP must record Founder-A Assortment BRAND authority"],
    [currentRoadmapSection, /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE:\s*NO/, "ROADMAP must record outlet-manager assortment manage NO"],
    [currentRoadmapSection, /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION:\s*AUTHORIZED_READ_OR_ESCALATE/, "ROADMAP must record assortment presentation rule"],
    ...
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE:\s*LOCKED/, "STATE must record IMP-036E architecture LOCKED"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/, "STATE must record IMP-036E architecture lock YES"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/, "STATE must record IMP-036E AUTHORIZED / STARTED / COMPLETE"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/, "STATE must record IMP-036E implementation authorized"],
    [currentStateAcceptance, /IMP-036E_STARTED:\s*YES/, "STATE must record IMP-036E started"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/, "STATE must record IMP-036E complete"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*NO/, "STATE must record IMP-036E unaccepted"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/, "STATE must record IMP-036E Founder UAT required"],
    [currentStateAcceptance, /IMP-036E_FOUNDER_UAT:\s*NOT_STARTED/, "STATE must record IMP-036E Founder UAT NOT_STARTED"],
    [currentStateAcceptance, /IMP036E_IMPLEMENTATION_EVIDENCE:\s*COMPLETE/, "STATE must record IMP-036E implementation evidence COMPLETE"],
    [currentStateAcceptance, /IMP_036E_INDEPENDENT_IMPLEMENTATION_REVIEW:\s*PASS/, "STATE must record IMP-036E independent implementation review PASS"],
    [currentStateAcceptance, /IMP036E_IMPLEMENTATION_MERGE_SHA:\s*0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739/, "STATE must record exact IMP-036E merge SHA"],
    [currentStateAcceptance, /IMP036E_IMPLEMENTATION_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/, "STATE must record exact IMP-036E merge tree"],
    [currentStateAcceptance, /IMP036E_REVIEWED_CANDIDATE_HEAD:\s*b2ffaa5bc2b5c6f58ff5241c226b583160132837/, "STATE must record reviewed candidate head"],
    [currentStateAcceptance, /IMP036E_REVIEWED_CANDIDATE_TREE:\s*0def59ce9bcab4575a7b43b7c3c07c85ac575428/, "STATE must record reviewed candidate tree"],
    [currentStateAcceptance, /IMP036E_ASSORTMENT_AUTHORITY:\s*BRAND/, "STATE must record Founder-A Assortment BRAND authority"],
    [currentStateAcceptance, /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE:\s*NO/, "STATE must record outlet-manager assortment manage NO"],
    [currentStateAcceptance, /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION:\s*AUTHORIZED_READ_OR_ESCALATE/, "STATE must record assortment presentation rule"],
    [currentStateAcceptance, /IMP036E_ASSORTMENT_WORKFORCE_TRANSPORT:\s*READ_ONLY_OPERATIONS_PROJECTION/, "STATE must record Assortment read-only Operations projection"],
  ...
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036E_IMPLEMENTATION_COMPLETION", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, artifactText]) {
    if (correctiveForbidden.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_CORRECTIVE_SEMANTICS",
        "IMP-036E completion must keep Assortment Store transport read-only, global session caps non-authoritative, and PIN non-authoritative",
      );
      break;
    }
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_PREMATURE_PROGRESSION",
        "IMP-036E completion must keep implementation unaccepted, Founder UAT not PASS, and IMP-036F unactivated",
      );
      break;
    }
  }
  ...
  if (
    !/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036E_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036E as IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE",
    );
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F Catalog, Menu, Pricing & Promotions Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E implementation completion");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20") {
    fail("IMP036E_ARCH_R20_CREATED", "ARCH-R20 must not be created during IMP-036E implementation completion");
  }
  if (!artifact) {
    fail("IMP036E_CAPABILITY_ARTIFACT", `Missing IMP-036E capability artifact at ${artifactRel}`);
  } else if (!artifactValidation.ok) {
  ...
  if (!experience) {
    ...
    const experienceRequired = [
      ...
      /IMP-036E_ACCEPTED:\s*NO/,
      /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/,
      /pendingAcceptance\s*=\s*IMP-036E/,
      /Implementation complete pending acceptance recorded at\s*GTM-R113 \/ STATE-R111/,
      /IMP036E_ASSORTMENT_AUTHORITY\s*=\s*BRAND/,
      /OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE\s*=\s*NO/,
      /OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION\s*=\s*AUTHORIZED_READ_OR_ESCALATE/,
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036E_EXPERIENCE_COMPLETION",
        "IMP-036E experience contract must record IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE with locked architecture facts",
      );
    }
    if (
      /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/.test(experienceText) ||
      /Lifecycle:\s*IMPLEMENTATION_IN_PROGRESS/.test(experienceText) ||
      /Implementation remains \*\*NOT_AUTHORIZED\*\* \/ \*\*NOT_STARTED\*\*/.test(experienceText) ||
      /IMP-036E_FOUNDER_UAT:\s*PASS/.test(experienceText)
    ) {
      fail("IMP036E_EXPERIENCE_STALE", "IMP-036E experience contract must not retain incomplete, not-authorized, or Founder-UAT-PASS status");
    }
  }
  ...
  if (
    state.meta.acceptedThrough !== "IMP-036D" ||
    state.meta.currentProductSlice !== "IMP-036E" ||
    state.meta.pendingAcceptance !== "IMP-036E" ||
    state.meta.nextProductSlice !== "IMP-036F"
  ) {
    fail(
      "IMP036E_STATE_POSITION",
      "STATE must record acceptedThrough IMP-036D, currentProductSlice IMP-036E, nextProductSlice IMP-036F, pendingAcceptance IMP-036E",
    );
  }
  if (
    !/IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) ||
    ...
    fail("IMP036E_IMP036D_REGRESSION", "IMP-036E completion must not regress or reopen IMP-036D acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E implementation completion");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E implementation completion");
  }

  const checkpoint = evaluateImp036eImplementationCompletionCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    currentProductImplementation: /Current Product Implementation:\s*IMP-036E/.test(currentStateActivity)
      ...
      : "",
    imp036d: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036e: /IMP-036E:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    architecture: /IMP-036E_ARCHITECTURE:\s*LOCKED/.test(currentRoadmapSection) ? "LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    implementation: /IMP-036E_IMPLEMENTATION:\s*AUTHORIZED \/ STARTED \/ COMPLETE/.test(currentRoadmapSection)
      ? "AUTHORIZED / STARTED / COMPLETE"
      : "",
    implementationAuthorized: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*YES/.test(currentStateAcceptance)
    ...
    started: /IMP-036E_STARTED:\s*YES/.test(currentRoadmapSection) &&
      ...
      ? "YES"
      : "",
    implementationComplete: /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentRoadmapSection) &&
      /IMP-036E_IMPLEMENTATION_COMPLETE:\s*YES/.test(currentStateAcceptance)
      ? "YES"
      : "",
    accepted: /IMP-036E_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    founderUatRequired: /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    founderUat: /IMP-036E_FOUNDER_UAT:\s*NOT_STARTED/.test(currentRoadmapSection) ? "NOT_STARTED" : "",
    imp036f: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036E:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentRoadmapSection)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    stateLifecycle: /IMP-036E:\s*IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE/.test(currentStateAcceptance)
      ? "IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
  ...
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note(`IMP-036E implementation complete pending acceptance (${artifactRel})`);
}

function checkImp036eArchitectureActivation(roadmap, state, architecture, decision) {
  if (!isImp036eArchitectureActivationCheckpoint(roadmap, state)) return;
  ...
  const futureSection = roadmap.text.split("## 5. Future GTM Slices")[1]?.split("## 6.")[0] || "";
  const capabilitiesDir = path.join(projectRoot, "docs/platform/capabilities");
  let imp036eCapabilityPresent = false;
  if (existsSync(capabilitiesDir)) {
    for (const name of readdirSync(capabilitiesDir)) {
      if (/^IMP-036E/i.test(name)) {
        imp036eCapabilityPresent = true;
        break;
      }
    }
  }
  ...
  const requiredTokens = [
    [currentRoadmapSection, /IMP-036E:\s*ARCHITECTURE_IN_PROGRESS/, "ROADMAP must record IMP-036E architecture in progress"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE:\s*NOT_LOCKED/, "ROADMAP must record IMP-036E architecture not locked"],
    [currentRoadmapSection, /IMP-036E_ARCHITECTURE_LOCKED:\s*NO/, "ROADMAP must record IMP-036E architecture lock NO"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/, "ROADMAP must record IMP-036E implementation not authorized"],
    [currentRoadmapSection, /IMP-036E_STARTED:\s*NO/, "ROADMAP must record IMP-036E not started"],
    [currentRoadmapSection, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "ROADMAP must record IMP-036E incomplete"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036E unaccepted"],
    ...
    [currentRoadmapSection, /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY:\s*NO/, "ROADMAP must record postal/PIN runtime authority NO"],
    [currentRoadmapSection, /SERVICEABILITY_MAP_IS_PROJECTION_ONLY:\s*YES/, "ROADMAP must record map projection-only"],
    [currentRoadmapSection, /D374_CREATED:\s*NO/, "ROADMAP must record D374_CREATED: NO"],
    [currentRoadmapSection, /ARCH_R20_CREATED:\s*NO/, "ROADMAP must record ARCH_R20_CREATED: NO"],
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "ROADMAP must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentRoadmapSection, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP-036E:\s*ARCHITECTURE_IN_PROGRESS/, "STATE must record IMP-036E architecture in progress"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE:\s*NOT_LOCKED/, "STATE must record IMP-036E architecture not locked"],
    [currentStateAcceptance, /IMP-036E_ARCHITECTURE_LOCKED:\s*NO/, "STATE must record IMP-036E architecture lock NO"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/, "STATE must record IMP-036E implementation not authorized"],
    [currentStateAcceptance, /IMP-036E_STARTED:\s*NO/, "STATE must record IMP-036E not started"],
    [currentStateAcceptance, /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/, "STATE must record IMP-036E incomplete"],
    [currentStateAcceptance, /IMP-036E_ACCEPTED:\s*NO/, "STATE must record IMP-036E unaccepted"],
    ...
    [currentStateAcceptance, /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY:\s*NO/, "STATE must record postal/PIN runtime authority NO"],
    [currentStateAcceptance, /SERVICEABILITY_MAP_IS_PROJECTION_ONLY:\s*YES/, "STATE must record map projection-only"],
    [currentStateAcceptance, /D374_CREATED:\s*NO/, "STATE must record D374_CREATED: NO"],
    [currentStateAcceptance, /ARCH_R20_CREATED:\s*NO/, "STATE must record ARCH_R20_CREATED: NO"],
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_ACTIVATED/, "STATE must keep IMP-036F PLANNED / NOT_ACTIVATED"],
    [currentStateAcceptance, /IMP-036D:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036D COMPLETE_AND_ACCEPTED"],
  ];
  for (const [text, pattern, message] of requiredTokens) {
    if (!pattern.test(text)) fail("IMP036E_ARCHITECTURE_ACTIVATION", message);
  }
  ...
  for (const text of [currentRoadmapSection, currentStateAcceptance, currentStateActivity]) {
    if (premature.some((pattern) => pattern.test(text))) {
      fail(
        "IMP036E_PREMATURE_PROGRESSION",
        "IMP-036E activation must not lock architecture, authorize/start implementation, or activate IMP-036F",
      );
      break;
    }
  }
  ...
  if (
    !/IMP-036E\s*\|\s*Store Operations Management\s*\|\s*ARCHITECTURE_IN_PROGRESS/.test(
      futureSection,
    )
  ) {
    fail(
      "IMP036E_ROADMAP_LIFECYCLE",
      "ROADMAP future ledger must list IMP-036E Store Operations Management as ARCHITECTURE_IN_PROGRESS",
    );
  }
  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_NOT_PLANNED", "ROADMAP future ledger must keep IMP-036F Catalog, Menu, Pricing & Promotions Management PLANNED");
  ...
  if (/\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? "")) {
    fail("IMP036E_D374_CREATED", "D-374 must not be created during IMP-036E architecture activation");
  }
  if (/architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? "")) {
    fail("IMP036E_ARCH_R20_CREATED", "ARCH-R20 must not be created during IMP-036E architecture activation");
  }
  if (imp036eCapabilityPresent) {
    fail(
      "IMP036E_CAPABILITY_ARTIFACT",
      "IMP-036E capability architecture artifact must not exist during architecture activation",
    );
  }
  let serviceabilityAligned = false;
  if (!experience) {
    fail("IMP036E_EXPERIENCE_MISSING", `Missing IMP-036E experience plan at ${experienceRel}`);
  } else {
    const experienceRequired = [
      ...
      /Implementation:\s*NOT_AUTHORIZED \/ NOT_STARTED/,
      /Founder UAT required:\s*YES/,
      /IMP-036E_ARCHITECTURE_WORK_AUTHORIZED:\s*YES/,
      /IMP-036E_ARCHITECTURE_LOCKED:\s*NO/,
      /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/,
      /IMP-036E_STARTED:\s*NO/,
      /IMP-036E_IMPLEMENTATION_COMPLETE:\s*NO/,
    ...
    if (experienceRequired.some((pattern) => !pattern.test(experienceText))) {
      fail(
        "IMP036E_EXPERIENCE_ACTIVATION",
        "IMP-036E experience plan must record architecture-in-progress activation and Serviceability alignment markers",
      );
    }
    const stalePinAuthority =
      /PIN-authoritative/i.test(experienceText) ||
      /postal-code-authoritative/i.test(experienceText) ||
      /PIN\/postal-code coverage/i.test(experienceText) ||
      /Serviceability stays PIN-authoritative/i.test(experienceText) ||
      /PIN coverage as runtime Serviceability/i.test(experienceText);
    if (stalePinAuthority) {
      fail(
        "IMP036E_STALE_PIN_SERVICEABILITY",
        "IMP-036E experience contract must not claim PIN/postal-code authoritative Serviceability",
      );
    } else {
      serviceabilityAligned =
        /OUTLET_DISTANCE_SERVICEABILITY_V1/.test(experienceText) &&
        /SERVICEABILITY_COORDINATE_AUTHORITY\s*=\s*YES/.test(experienceText) &&
        /SERVICEABILITY_POSTAL_PIN_RUNTIME_AUTHORITY\s*=\s*NO/.test(experienceText) &&
        /SERVICEABILITY_MAP_IS_PROJECTION_ONLY\s*=\s*YES/.test(experienceText);
      if (!serviceabilityAligned) {
        fail(
          "IMP036E_SERVICEABILITY_ALIGNMENT",
          "IMP-036E experience contract must align to accepted OUTLET_DISTANCE_SERVICEABILITY_V1 markers",
        );
      }
    }
  }
  ...
  if (!/IMP-036E ARCHITECTURE_IN_PROGRESS/.test(currentStateActivity)) {
    fail("IMP036E_STATE_ACTIVITY", "STATE current governance activity must record IMP-036E ARCHITECTURE_IN_PROGRESS");
  }
  if (
    !/IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) ||
    /IMP-036D_ACCEPTED:\s*NO/.test(currentStateAcceptance)
  ) {
    fail("IMP036E_IMP036D_REGRESSION", "IMP-036E activation must not regress or reopen IMP-036D acceptance");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036E_ARCH_VERSION", "ARCHITECTURE must be ARCH-R19 during IMP-036E architecture activation");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036E_DR_VERSION", "decision register must be DR-15 during IMP-036E architecture activation");
  }

  const checkpoint = evaluateImp036eArchitectureActivationCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036d: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036e: /IMP-036E:\s*ARCHITECTURE_IN_PROGRESS/.test(currentRoadmapSection) ? "ARCHITECTURE_IN_PROGRESS" : "",
    architecture: /IMP-036E_ARCHITECTURE:\s*NOT_LOCKED/.test(currentRoadmapSection) ? "NOT_LOCKED" : "",
    architectureLocked: /IMP-036E_ARCHITECTURE_LOCKED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    implementation: /IMP-036E_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) &&
      /IMP-036E_STARTED:\s*NO/.test(currentRoadmapSection)
      ? "NOT_AUTHORIZED / NOT_STARTED"
      : "",
    ...
    accepted: /IMP-036E_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    imp036f: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    roadmapLifecycle: /IMP-036E:\s*ARCHITECTURE_IN_PROGRESS/.test(currentRoadmapSection)
      ? "ARCHITECTURE_IN_PROGRESS"
      : "",
    stateLifecycle: /IMP-036E:\s*ARCHITECTURE_IN_PROGRESS/.test(currentStateAcceptance)
      ? "ARCHITECTURE_IN_PROGRESS"
      : "",
    architectureVersion: architecture?.meta.architectureVersion,
    decisionRegisterVersion: decision?.meta.decisionRegisterVersion,
    founderUatRequired: /IMP-036E_FOUNDER_UAT_REQUIRED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    d374Exists: /\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? ""),
    archR20Exists: /architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") ||
      /ARCH-R20/.test(architecture?.meta?.architectureVersion ?? ""),
    capabilityArtifactExists: imp036eCapabilityPresent,
    imp036dAccepted: /IMP-036D:\s*COMPLETE_AND_ACCEPTED/.test(currentStateAcceptance) &&
      /IMP-036D_ACCEPTED:\s*YES/.test(currentStateAcceptance),
    serviceabilityAligned,
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note("IMP-036E architecture activation lifecycle valid");
}
...
function checkProductDeliveryProcessAuthorities() {
  ...
  if (testing) {
    ...
    for (const marker of TESTING_PROSPECTIVE_MARKERS) {
        ...
        for (const [journeyId, expectedStatus] of requiredGjStatuses) {
          ...
          if (actual !== expectedStatus) {
            ...
            fail(
              ...
              `${journeyId} must be ${expectedStatus} at acceptedThrough IMP-036E / IMP036F_ACTIVATED YES (got ${actual ?? "NOT_FOUND"})`,
            );
          }
        }
        if (allAligned) {
...
export function runProjectConsistency() {
  ...
  checkImp036dImplementationStart(roadmap, state, architecture, decision);
  checkImp036dImplementationCompletion(roadmap, state, architecture, decision);
  checkImp036dAcceptance(roadmap, state, architecture, decision);
  checkImp036eArchitectureActivation(roadmap, state, architecture, decision);
  checkImp036eArchitectureLock(roadmap, state, architecture, decision);
  ...
  checkImp036eAcceptance(roadmap, state, architecture, decision);
...
/**
 * CURRENT checkpoint: IMP-036F product-slice ADVANCE after IMP-036E acceptance.
 * Does not create Product Definition, lock architecture, or authorize implementation.
 */
function checkImp036fActivation(roadmap, state, architecture, decision) {
  if (!isImp036fActivationCheckpoint(roadmap, state)) return;
  ...
  if (productDef) {
    fail("IMP036F_PRODUCT_DEFINITION_EXISTS", "IMP-036F Product Definition must not exist at ADVANCE activation");
  }

  const requiredTokens = [
    [currentRoadmapSection, /IMP-036E:\s*COMPLETE_AND_ACCEPTED/, "ROADMAP must preserve IMP-036E COMPLETE_AND_ACCEPTED"],
    [currentRoadmapSection, /IMP-036E_ACCEPTED:\s*YES/, "ROADMAP must preserve IMP-036E accepted"],
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "ROADMAP must keep IMP-036F formal lifecycle PLANNED"],
    [currentRoadmapSection, /IMP036F_ACTIVATED:\s*YES/, "ROADMAP must record IMP036F_ACTIVATED: YES"],
    [currentRoadmapSection, /IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/, "ROADMAP must record Product Definition NOT_CREATED"],
    [currentRoadmapSection, /IMP036F_ARCHITECTURE_LOCKED:\s*NO/, "ROADMAP must record architecture not locked"],
    ...
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "STATE must keep IMP-036F formal lifecycle PLANNED"],
    [currentStateAcceptance, /IMP036F_ARCHITECTURE_LOCKED:\s*NO/, "STATE must record architecture not locked"],
    ...
    [currentSliceSection, /IMP036F_ACTIVATED:\s*YES/, "ROADMAP current slice section must record activation"],
    [currentSliceSection, /formal ROADMAP lifecycle for IMP-036F remains `PLANNED`/i, "ROADMAP current slice must keep formal lifecycle PLANNED"],
  ];
  for (const [haystack, pattern, message] of requiredTokens) {
    if (!pattern.test(haystack)) fail("IMP036F_ACTIVATION", message);
  }

  if (/IMP036F_ACTIVATED:\s*NO/.test(currentRoadmapSection) || /IMP036F_ACTIVATED:\s*NO/.test(currentStateAcceptance)) {
    fail("IMP036F_ACTIVATION_STALE", "CURRENT ROADMAP/STATE must not claim IMP036F_ACTIVATED: NO after activation");
  }
  if (/currentProductSlice:\s*NONE/i.test(currentRoadmapSection) || /Current Product Slice:\s*NONE/i.test(currentRoadmapSection)) {
    fail("IMP036F_ACTIVATION_STALE", "CURRENT ROADMAP must not claim currentProductSlice NONE after IMP-036F activation");
  }
  ...
  const forbidden = [
    ...
    /IMP036F_ARCHITECTURE_LOCKED:\s*YES/,
    /IMP-036F:\s*ARCHITECTURE_IN_PROGRESS/,
    /IMP-036F:\s*ARCHITECTURE_LOCKED/,
    /IMP-036F:\s*IMPLEMENTATION_IN_PROGRESS/,
  ...
  for (const haystack of [currentRoadmapSection, currentStateAcceptance, currentStateActivity, currentSliceSection]) {
    if (forbidden.some((pattern) => pattern.test(haystack))) {
      fail("IMP036F_PREMATURE_PROGRESSION", "IMP-036F activation must not authorize/start implementation, lock architecture, or activate IMP-036G");
      break;
    }
  }

  if (!/IMP-036F\s*\|\s*Catalog, Menu, Pricing & Promotions Management\s*\|\s*PLANNED/.test(futureSection)) {
    fail("IMP036F_ROADMAP_LIFECYCLE", "ROADMAP future ledger must keep IMP-036F PLANNED");
  }
  ...
  if (!/IMP-036F activated as CURRENT product slice/.test(currentStateActivity) && !/IMP036F_ACTIVATED:\s*YES/.test(currentStateActivity)) {
    fail("IMP036F_STATE_ACTIVITY", "STATE current governance activity must record IMP-036F activation");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036F_ARCH_VERSION", "ARCHITECTURE must remain ARCH-R19 during IMP-036F activation");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036F_DR_VERSION", "decision register must remain DR-15 during IMP-036F activation");
  }
  ...
  if (productDelivery && productDelivery.meta.version !== "PD-1") {
    fail("IMP036F_PD_VERSION", "PRODUCT-DELIVERY must remain PD-1 during IMP-036F activation");
  }

  const checkpoint = evaluateImp036fActivationCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    pendingAcceptance: state.meta.pendingAcceptance,
    imp036e: /IMP-036E:\s*COMPLETE_AND_ACCEPTED/.test(currentRoadmapSection) ? "COMPLETE_AND_ACCEPTED" : "",
    imp036fFormalLifecycle: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    imp036fActivated: /IMP036F_ACTIVATED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    architectureLocked: /IMP036F_ARCHITECTURE_LOCKED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    ...
    productDefinition: /IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/.test(currentRoadmapSection) ? "NOT_CREATED" : "",
    ...
    productDefinitionExists: productDef !== null,
    d374Exists: /\|\s*D-374\s*\|/.test(decision?.text ?? "") || /###\s*D-374\b/.test(decision?.text ?? ""),
    archR20Exists: /architectureVersion":\s*"ARCH-R20"/.test(architecture?.text ?? "") || architecture?.meta?.architectureVersion === "ARCH-R20",
  });
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
  else note("IMP-036F product-slice activation lifecycle valid (PD-1 ANCHOR/DISCOVER next; no Product Definition)");
}

/**
 * CURRENT checkpoint: IMP-036F post-discovery Product Definition draft authorization (R117/S115).
 * Allows Product Definition file absent or present as ungated DRAFT with gate NOT_PERFORMED.
 */
function checkImp036fProductDefinitionDraftAuthorized(roadmap, state, architecture, decision) {
  if (!isImp036fProductDefinitionDraftAuthorizedCheckpoint(roadmap, state)) return;
  ...
  const requiredTokens = [
    ...
    [currentRoadmapSection, /IMP-036F:\s*PLANNED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "ROADMAP must keep IMP-036F formal lifecycle PLANNED"],
    [currentRoadmapSection, /IMP036F_ACTIVATED:\s*YES/, "ROADMAP must record IMP036F_ACTIVATED: YES"],
    [currentRoadmapSection, /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/, "ROADMAP must record Product Definition DRAFT_AUTHORIZED"],
    [currentRoadmapSection, /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/, "ROADMAP must record Product Definition Gate NOT_PERFORMED"],
    [currentRoadmapSection, /IMP036F_ARCHITECTURE_LOCKED:\s*NO/, "ROADMAP must record architecture not locked"],
    [currentRoadmapSection, /IMP036F_IMPLEMENTATION_AUTHORIZED:\s*NO/, "ROADMAP must record implementation not authorized"],
    [currentRoadmapSection, /IMP036F_STARTED:\s*NO/, "ROADMAP must record IMP-036F not started"],
    [currentRoadmapSection, /IMP036F_ACCEPTED:\s*NO/, "ROADMAP must record IMP-036F unaccepted"],
    ...
    [currentStateAcceptance, /IMP-036E:\s*COMPLETE_AND_ACCEPTED/, "STATE must preserve IMP-036E COMPLETE_AND_ACCEPTED"],
    [currentStateAcceptance, /IMP036F_ACTIVATED:\s*YES/, "STATE must record IMP036F_ACTIVATED: YES"],
    [currentStateAcceptance, /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/, "STATE must record Product Definition DRAFT_AUTHORIZED"],
    [currentStateAcceptance, /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/, "STATE must record Product Definition Gate NOT_PERFORMED"],
    [currentStateAcceptance, /IMP-036F:\s*PLANNED \/ NOT_AUTHORIZED \/ NOT_STARTED/, "STATE must keep IMP-036F formal lifecycle PLANNED"],
    [currentStateAcceptance, /IMP036F_ARCHITECTURE_LOCKED:\s*NO/, "STATE must record architecture not locked"],
    [currentStateAcceptance, /IMP036F_IMPLEMENTATION_AUTHORIZED:\s*NO/, "STATE must record implementation not authorized"],
    [currentStateAcceptance, /IMP036F_STARTED:\s*NO/, "STATE must record not started"],
    ...
    [currentStateAcceptance, /IMP036G_ACTIVATED:\s*NO/, "STATE must keep IMP-036G unactivated"],
    [currentSliceSection, /IMP036F_ACTIVATED:\s*YES/, "ROADMAP current slice section must record activation"],
    [currentSliceSection, /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/, "ROADMAP current slice must record DRAFT_AUTHORIZED"],
    [currentSliceSection, /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/, "ROADMAP current slice must record gate NOT_PERFORMED"],
    [currentSliceSection, /formal ROADMAP lifecycle for IMP-036F remains `PLANNED`/i, "ROADMAP current slice must keep formal lifecycle PLANNED"],
    [currentStateActivity, /DRAFT_AUTHORIZED/, "STATE current governance activity must record DRAFT_AUTHORIZED"],
    [currentStateActivity, /NOT_PERFORMED/, "STATE current governance activity must record Product Definition Gate NOT_PERFORMED"],
  ];
  for (const [haystack, pattern, message] of requiredTokens) {
    if (!pattern.test(haystack)) fail("IMP036F_PD_DRAFT_AUTHORIZED", message);
  }

  if (/IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/.test(currentRoadmapSection) || /IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/.test(currentStateAcceptance)) {
    fail("IMP036F_PD_DRAFT_AUTHORIZED_STALE", "CURRENT ROADMAP/STATE must not claim Product Definition NOT_CREATED after draft authorization");
  }
  if (/IMP036F_ACTIVATED:\s*NO/.test(currentRoadmapSection) || /IMP036F_ACTIVATED:\s*NO/.test(currentStateAcceptance)) {
    fail("IMP036F_ACTIVATION_STALE", "CURRENT ROADMAP/STATE must not claim IMP036F_ACTIVATED: NO after activation");
  }
  ...
  const forbidden = [
    ...
    /IMP036G_ACTIVATED:\s*YES/,
    /IMP-036G:\s*ARCHITECTURE_IN_PROGRESS/,
    /IMP036F_PRODUCT_DEFINITION_GATE:\s*PASS/,
    /IMP036F_PRODUCT_DEFINITION:\s*APPROVED/,
    /IMP036F_PRODUCT_DEFINITION:\s*CREATED/,
  ];
  for (const haystack of [currentRoadmapSection, currentStateAcceptance, currentStateActivity, currentSliceSection]) {
    if (forbidden.some((pattern) => pattern.test(haystack))) {
      fail(
        "IMP036F_PREMATURE_PROGRESSION",
        "IMP-036F draft authorization must not pass Product Definition Gate, lock architecture, authorize/start implementation, or activate IMP-036G",
      );
      break;
    }
  }
  ...
  if (roadmap.meta.roadmapVersion !== "GTM-R117" || state.meta.stateVersion !== "STATE-R115") {
    fail("IMP036F_PD_DRAFT_AUTHORIZED_VERSION", "ROADMAP/STATE must be GTM-R117 / STATE-R115 at draft-authorized checkpoint");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036F_ARCH_VERSION", "ARCHITECTURE must remain ARCH-R19 during IMP-036F draft authorization");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036F_DR_VERSION", "decision register must remain DR-15 during IMP-036F draft authorization");
  }

  const productDelivery = loadCanonical("docs/platform/PRODUCT-DELIVERY.md", "PRODUCT_DELIVERY_PROCESS", ["version"]);
  if (productDelivery && productDelivery.meta.version !== "PD-1") {
    fail("IMP036F_PD_VERSION", "PRODUCT-DELIVERY must remain PD-1 during IMP-036F draft authorization");
  }

  const checkpoint = evaluateImp036fProductDefinitionDraftAuthorizedCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
    ...
    imp036fFormalLifecycle: /IMP-036F:\s*PLANNED/.test(currentRoadmapSection) ? "PLANNED" : "",
    imp036fActivated: /IMP036F_ACTIVATED:\s*YES/.test(currentRoadmapSection) ? "YES" : "",
    productDefinition: /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/.test(currentRoadmapSection) ? "DRAFT_AUTHORIZED" : "",
    productDefinitionGate: /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/.test(currentRoadmapSection) ? "NOT_PERFORMED" : "",
    architectureLocked: /IMP036F_ARCHITECTURE_LOCKED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    implementationAuthorized: /IMP036F_IMPLEMENTATION_AUTHORIZED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    started: /IMP036F_STARTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
    accepted: /IMP036F_ACCEPTED:\s*NO/.test(currentRoadmapSection) ? "NO" : "",
  ...
  if (!checkpoint.ok) fail(checkpoint.code, checkpoint.message);
    ...
    note(
      productDefAbs
        ? "IMP-036F Product Definition draft authorization valid (ungated DRAFT candidate present; gate NOT_PERFORMED)"
        : "IMP-036F Product Definition draft authorization valid (DRAFT_AUTHORIZED; Product Definition file absent; gate NOT_PERFORMED)",
    );
  }
}

/**
 * CURRENT checkpoint: IMP-036F Product Definition Gate PASS persistence (R118/S116).
 * Requires approved PD with gate PERFORMED/PASS; Architecture Fit remains NOT_PERFORMED.
 */
function checkImp036fProductDefinitionGatePass(roadmap, state, architecture, decision) {
  if (!isImp036fProductDefinitionGatePassCheckpoint(roadmap, state)) return;
  ...
  if (!productDefAbs) {
    fail("IMP036F_PD_APPROVED_ABSENT", "IMP-036F Product Definition must exist at Product Definition Gate PASS checkpoint");
  }
  ...
  const requiredTokens = [
    ...
    [currentStateAcceptance, /IMP036G_ACTIVATED:\s*NO/, "STATE must keep IMP-036G unactivated"],
    [currentSliceSection, /IMP036F_ACTIVATED:\s*YES/, "ROADMAP current slice section must record activation"],
    [currentSliceSection, /IMP036F_PRODUCT_DEFINITION:\s*APPROVED/, "ROADMAP current slice must record APPROVED"],
    [currentSliceSection, /IMP036F_PRODUCT_DEFINITION_GATE:\s*PASS/, "ROADMAP current slice must record gate PASS"],
    [currentSliceSection, /formal ROADMAP lifecycle for IMP-036F remains `PLANNED`/i, "ROADMAP current slice must keep formal lifecycle PLANNED"],
    [currentStateActivity, /APPROVED/, "STATE current governance activity must record Product Definition APPROVED"],
    [currentStateActivity, /IMP036F_PRODUCT_DEFINITION_GATE:\s*PASS|Product Definition Gate PASS/i, "STATE current governance activity must record Product Definition Gate PASS"],
    [currentStateActivity, /ARCHITECTURE_FIT — NOT_PERFORMED|Architecture Fit NOT performed/i, "STATE current governance activity must record Architecture Fit NOT_PERFORMED"],
  ];
  for (const [haystack, pattern, message] of requiredTokens) {
    if (!pattern.test(haystack)) fail("IMP036F_PD_GATE_PASS", message);
  }

  if (/IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/.test(currentRoadmapSection) || /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/.test(currentStateAcceptance)) {
    fail("IMP036F_PD_GATE_PASS_STALE", "CURRENT ROADMAP/STATE must not claim Product Definition DRAFT_AUTHORIZED after gate PASS");
  }
  if (/IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/.test(currentRoadmapSection) || /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/.test(currentStateAcceptance)) {
    fail("IMP036F_PD_GATE_PASS_STALE", "CURRENT ROADMAP/STATE must not claim Product Definition Gate NOT_PERFORMED after gate PASS");
  }
  if (/IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/.test(currentRoadmapSection) || /IMP036F_PRODUCT_DEFINITION:\s*NOT_CREATED/.test(currentStateAcceptance)) {
    fail("IMP036F_PD_GATE_PASS_STALE", "CURRENT ROADMAP/STATE must not claim Product Definition NOT_CREATED after gate PASS");
  }
  if (/IMP036F_ACTIVATED:\s*NO/.test(currentRoadmapSection) || /IMP036F_ACTIVATED:\s*NO/.test(currentStateAcceptance)) {
    fail("IMP036F_ACTIVATION_STALE", "CURRENT ROADMAP/STATE must not claim IMP036F_ACTIVATED: NO after activation");
  }
  ...
  const forbidden = [
    ...
    /IMP036G_ACTIVATED:\s*YES/,
    /IMP-036G:\s*ARCHITECTURE_IN_PROGRESS/,
    /IMP036F_PRODUCT_DEFINITION:\s*DRAFT_AUTHORIZED/,
    /IMP036F_PRODUCT_DEFINITION_GATE:\s*NOT_PERFORMED/,
  ];
  for (const haystack of [currentRoadmapSection, currentStateAcceptance, currentStateActivity, currentSliceSection]) {
    if (forbidden.some((pattern) => pattern.test(haystack))) {
      fail(
        "IMP036F_PREMATURE_PROGRESSION",
        "IMP-036F Product Definition Gate PASS must not retain draft/ungated markers, lock architecture, authorize/start implementation, or activate IMP-036G",
      );
      break;
    }
  }
  ...
  if (roadmap.meta.roadmapVersion !== "GTM-R118" || state.meta.stateVersion !== "STATE-R116") {
    fail("IMP036F_PD_GATE_PASS_VERSION", "ROADMAP/STATE must be GTM-R118 / STATE-R116 at Product Definition Gate PASS checkpoint");
  }
  if (architecture?.meta.architectureVersion !== "ARCH-R19") {
    fail("IMP036F_ARCH_VERSION", "ARCHITECTURE must remain ARCH-R19 during IMP-036F Product Definition Gate PASS persistence");
  }
  if (decision?.meta.decisionRegisterVersion !== "DR-15") {
    fail("IMP036F_DR_VERSION", "decision register must remain DR-15 during IMP-036F Product Definition Gate PASS persistence");
  }

  const productDelivery = loadCanonical("docs/platform/PRODUCT-DELIVERY.md", "PRODUCT_DELIVERY_PROCESS", ["version"]);
  if (productDelivery && productDelivery.meta.version !== "PD-1") {
    fail("IMP036F_PD_VERSION", "PRODUCT-DELIVERY must remain PD-1 during IMP-036F Product Definition Gate PASS persistence");
  }

  const checkpoint = evaluateImp036fProductDefinitionGatePassCheckpoint({
    roadmapVersion: roadmap.meta.roadmapVersion,
    stateVersion: state.meta.stateVersion,
    acceptedThrough: state.meta.acceptedThrough,
    currentProductSlice: state.meta.currentProductSlice,
