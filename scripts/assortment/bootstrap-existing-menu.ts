...
/**
 * Bootstrap Brand assortment includes from existing-menu-v1 (IMP-014).
 *
...
function parseArgs(argv: readonly string[]): { apply: boolean } {
  ...
  for (const arg of argv) {
    if (arg === "--apply") apply = true;
      ...
      throw new AssortmentBootstrapError("validation", `Unsupported flag: ${arg}`);
    }
  }
  return { apply };
}
  ...
  try {
    ...
    process.stdout.write(
      `${JSON.stringify({
        ...
        derivedVariantCount: result.derivedVariantCount,
