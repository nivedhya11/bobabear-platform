#!/usr/bin/env node
// Live image/container inspection for the `workforce-auth` service (IMP-010).
// Requires Docker Desktop and an already-built `boba-bear-workforce-auth:local`
// image / running `workforce-auth` container. Uses `docker` CLI child
// processes; never prints a full container environment — only key names or
// safe booleans, matching scripts/docker/inspect-runtime.mjs (IMP-005A) and
// scripts/docker/customer-auth-inspect.mjs (IMP-009).
//
// Usage: node scripts/docker/workforce-auth-inspect.mjs
...
/** The workforce-auth service *does* need `BOBA_BEAR_DATABASE_URL` (the
...
function inspectImage(results) {
  let inspect;
  try {
    inspect = dockerJson(["image", "inspect", "boba-bear-workforce-auth:local"])[0];
  } catch {
    record(
      results,
      "boba-bear-workforce-auth:local image exists",
      false,
      "run `npm run docker:build` first",
    );
    return;
  }
  record(results, "boba-bear-workforce-auth:local image exists", true, inspect.Id);
  ...
  record(results, "Image exposes only port 8082/tcp", exposedPorts.length === 1 && exposedPorts[0] === "8082/tcp", exposedPorts.join(", "));
  ...
  try {
    docker([
      ...
      "--entrypoint",
      "/bin/sh",
      "boba-bear-workforce-auth:local",
      "-c",
  ...
  try {
    docker([
      ...
      "boba-bear-workforce-auth:local",
      "-c",
      "test ! -d /app/src && test ! -d /app/tests && test ! -e /app/.env && test ! -e /app/.env.local && exit 0 || exit 1",
    ]);
    noSourceTree = true;
  } catch {
    noSourceTree = false;
  }
  record(results, "Image contains no TypeScript source, tests, or .env files", noSourceTree);
}

function inspectRunningWorkforceAuthContainer(results) {
  let containerId;
  try {
    containerId = docker(["compose", "ps", "-q", "workforce-auth"]);
  } catch {
    containerId = "";
  }
  if (!containerId) {
    record(results, "workforce-auth container is running", false, "run `npm run docker:up` first");
    return;
  }
  record(results, "workforce-auth container is running", true);
  ...
  record(results, "Container does not use host networking", hostConfig.NetworkMode !== "host", hostConfig.NetworkMode);
...
function main() {
  ...
  inspectRunningWorkforceAuthContainer(results);

  console.log("docker:workforce-auth:inspect — image and runtime evidence");
