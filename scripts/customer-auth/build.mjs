#!/usr/bin/env node
// Customer-auth service build (IMP-009).
//
// Compiles `src/server/customer-auth/main.ts` (and everything it imports —
// `src/server/auth/**`, `src/server/persistence/**`,
// `src/platform/{config,database}/**`, `src/shared/customer-auth/**`) to
// plain ESM JavaScript under `dist-customer-auth/`, then rewrites every
...
//
// Usage: node scripts/customer-auth/build.mjs
...
const outDir = path.join(projectRoot, "dist-customer-auth");
...
export function resolveRewrittenSpecifier(fileAbsolutePath, specifier, outDirAbsolutePath) {
  ...
  if (!resolved) {
    throw new Error(
      `customer-auth build: could not resolve "${specifier}" imported from ` +
...
function runTsc() {
  ...
  execFileSync(process.execPath, [tscBin, "-p", "tsconfig.customer-auth.json"], {
    cwd: projectRoot,
    stdio: "inherit",
  });
}
...
function main() {
  ...
  console.log(`customer-auth/build: compiled and rewrote ${rewrittenCount} file(s) into dist-customer-auth/.`);
}
