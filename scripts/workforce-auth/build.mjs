#!/usr/bin/env node
// Workforce-auth service build (IMP-010).
//
// Compiles `src/server/workforce-auth/main.ts` (and everything it imports —
// `src/server/auth/**`, `src/server/persistence/**`,
// `src/platform/{config,database}/**`, `src/shared/workforce-auth/**`) to
// plain ESM JavaScript under `dist-workforce-auth/`, then rewrites every
...
//
// Usage: node scripts/workforce-auth/build.mjs
...
const outDir = path.join(projectRoot, "dist-workforce-auth");
...
export function resolveRewrittenSpecifier(fileAbsolutePath, specifier, outDirAbsolutePath) {
  ...
  if (!resolved) {
    throw new Error(
      `workforce-auth build: could not resolve "${specifier}" imported from ` +
...
function runTsc() {
  ...
  execFileSync(process.execPath, [tscBin, "-p", "tsconfig.workforce-auth.json"], {
    cwd: projectRoot,
    stdio: "inherit",
  });
}
...
function main() {
  ...
  console.log(`workforce-auth/build: compiled and rewrote ${rewrittenCount} file(s) into dist-workforce-auth/.`);
}
