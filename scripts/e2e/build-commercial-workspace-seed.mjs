#!/usr/bin/env node
/** Compile the IMP-036F F6B commercial workspace E2E seed into a caller-owned disposable ESM graph. */
...
const entryRelativePath = path.join("scripts", "e2e", "seed-commercial-workspace.js");
...
const ALLOWED_TEMP_PREFIXES = ["imp036fe2e_", "imp036f-runtime-asset-repair-"];
...
function assertSafeOutputRoot(outputRoot) {
  if (typeof outputRoot !== "string" || !path.isAbsolute(outputRoot)) {
    throw new Error("commercial workspace seed build: outputRoot must be an absolute disposable path.");
  }
  ...
  if (
    ...
    throw new Error("commercial workspace seed build: outputRoot must be below an approved /tmp disposable directory.");
  }
  if (existsSync(outputRoot)) {
    throw new Error("commercial workspace seed build: caller must provide a new disposable outputRoot.");
  }
}
...
function hasJsonAttribute(owner, sourceFile) {
  ...
  if (elements.length !== 1 || elements[0].name.text !== "type" || elements[0].value.text !== "json") {
    throw new Error(`commercial workspace seed build: incompatible JSON import attributes in ${sourceFile.fileName}.`);
  }
  return true;
}
...
function resolveSpecifier(file, specifier, outputRoot) {
  ...
  if (!resolved) throw new Error(`commercial workspace seed build: unresolved emitted import "${specifier}" in ${path.relative(outputRoot, file)}.`);
...
function rewriteSpecifiers(file, outputRoot) {
  ...
  const replacements = moduleSpecifiers(source, file).flatMap(({ node, owner, kind }) => {
    ...
    if (kind === "dynamic") {
      if (owner.arguments.length !== 1) throw new Error(`commercial workspace seed build: incompatible JSON dynamic import attributes in ${file}.`);
...
function validateOutput(outputRoot, files) {
  ...
  if (!existsSync(entryPath)) throw new Error("commercial workspace seed build: compiled seed entry is missing.");
  if (!existsSync(path.join(outputRoot, "package.json"))) throw new Error("commercial workspace seed build: ESM package marker is missing.");
  ...
  for (const file of files) {
    for (const { node, owner, kind } of moduleSpecifiers(readFileSync(file, "utf8"), file)) {
      if (node.text.startsWith("@/")) {
        ...
        throw new Error(`commercial workspace seed build: alias import remains in ${path.relative(outputRoot, file)}.`);
      }
  ...
  if (json.missing || json.dynamicUnsupported || json.targetsMissing || json.targetsOutsideRoot || validation.executableTsSourceImports) throw new Error(`commercial workspace seed build: validation failed (${JSON.stringify({ json, validation })}).`);
...
function assertCompiledRootContainment(compiledRoot, destinationPath) {
  ...
  if (!isPathContainedWithin(resolvedCompiledRoot, resolvedDestination)) {
    throw new Error(`commercial workspace seed build: runtime asset destination escapes compiled root (${path.relative(compiledRoot, destinationPath)}).`);
  }
}

function assertRegularRepositorySource(sourcePath, permittedRoot) {
  if (!existsSync(sourcePath)) {
    throw new Error(`commercial workspace seed build: missing runtime asset source ${path.relative(root, sourcePath)}.`);
  }
  const stat = lstatSync(sourcePath);
  if (stat.isSymbolicLink()) {
    throw new Error(`commercial workspace seed build: runtime asset source must not be a symlink (${path.relative(root, sourcePath)}).`);
  }
  if (!stat.isFile()) {
    throw new Error(`commercial workspace seed build: runtime asset source must be a regular file (${path.relative(root, sourcePath)}).`);
  }
  ...
  if (!isPathContainedWithin(resolvedPermittedRoot, resolvedSource)) {
    throw new Error(`commercial workspace seed build: runtime asset source escapes repository root (${path.relative(root, sourcePath)}).`);
  }
}
...
function assertFixedAssetDestinationAllowed(relativePath, destinationPath) {
  ...
  if (relativePath.endsWith(".ts")) {
    if (existsSync(destinationPath) && lstatSync(destinationPath).isFile()) {
      throw new Error(`commercial workspace seed build: unexpected fixed runtime asset collision at ${relativePath}.`);
    }
    return;
  }
...
function normalizeManifestPublicRelativePath(imagePath, entrySourceKey) {
  if (typeof imagePath !== "string" || imagePath.length === 0) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} has an empty image_path.`);
  }
  if (imagePath.includes("\0")) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} image_path contains NUL.`);
  }
  if (path.isAbsolute(imagePath) && !imagePath.startsWith("/")) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} image_path is an absolute filesystem path.`);
  }
  const trimmed = imagePath.startsWith("/") ? imagePath.slice(1) : imagePath;
  if (trimmed.includes("..") || path.isAbsolute(trimmed)) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} image_path contains traversal.`);
  }
  if (!trimmed.startsWith("assets/menu/")) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} image_path is outside public/assets/menu/.`);
  }
  const relativePath = path.posix.join("public", trimmed);
  if (relativePath.endsWith("/")) {
    throw new Error(`commercial workspace seed build: manifest entry ${entrySourceKey} image_path refers to a directory.`);
  }
...
function derivePublicAssetRelativePaths() {
  ...
  if (!Array.isArray(entries)) {
    throw new Error("commercial workspace seed build: manifest entries must be an array.");
  }
  ...
  for (const entry of entries) {
    ...
    if (previous !== undefined && previous !== entry.image_path) {
      throw new Error(`commercial workspace seed build: ambiguous duplicate public asset path ${normalized}.`);
    }
  ...
  if (entries.length !== EXPECTED_PUBLIC_UNIQUE || relativePaths.length !== EXPECTED_PUBLIC_UNIQUE) {
    throw new Error(`commercial workspace seed build: expected ${EXPECTED_PUBLIC_UNIQUE} unique public assets, found ${relativePaths.length} (${entries.length} manifest references).`);
  }
  for (const relativePath of relativePaths) {
    if (!relativePath.startsWith(PUBLIC_MENU_PREFIX)) {
      throw new Error(`commercial workspace seed build: derived public asset outside allowlist (${relativePath}).`);
    }
  }
...
function copyRuntimeAsset(relativePath, compiledRoot, destinationPaths) {
  ...
  if (destinationPaths.has(destinationPath)) {
    throw new Error(`commercial workspace seed build: duplicate runtime asset destination ${relativePath}.`);
  }
  ...
  if (sourceMeta.size !== destinationMeta.size || sourceMeta.sha256 !== destinationMeta.sha256) {
    throw new Error(`commercial workspace seed build: runtime asset hash mismatch for ${relativePath}.`);
  }
...
function packageRuntimeAssets(compiledRoot) {
  ...
  for (const relativePath of relativePaths) {
    if (existsSync(path.join(compiledRoot, relativePath))) {
      throw new Error(`commercial workspace seed build: unexpected public asset collision at ${relativePath}.`);
    }
    const result = copyRuntimeAsset(relativePath, compiledRoot, destinationPaths);
    copied.push(result);
    totalBytes += result.bytes;
  ...
  if (packaging.totalCopied !== EXPECTED_TOTAL_RUNTIME_ASSETS || packaging.missing !== 0) {
    throw new Error(`commercial workspace seed build: runtime asset closure incomplete (${JSON.stringify(packaging)}).`);
  }
...
export function buildCommercialWorkspaceSeed({ outputRoot }) {
  assertSafeOutputRoot(outputRoot);
  mkdirSync(outputRoot, { recursive: true, mode: 0o700 });
  execFileSync(process.execPath, [path.join(root, "node_modules", "typescript", "bin", "tsc"), "-p", "tsconfig.commercial-workspace-e2e-seed.json", "--outDir", outputRoot], { cwd: root, stdio: "inherit" });
