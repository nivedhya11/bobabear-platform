...
import {
  evaluateScheduledHorizonServiceability,
...
/**
 * Delivery Outlet for a Scheduled window.
 * Same geographic authority as ASAP serviceability. Current pause and
 * current closed-by-schedule do not reject the Outlet.
 */
export async function resolveScheduledDeliveryOutlet(
  persistence: Persistence,
  brandId: string,
  destination: CheckoutDestination,
  clock: CheckoutClock,
  ...
  let result;
  try {
    result = await evaluateScheduledHorizonServiceability(
      persistence,
      {
        brandId,
        location: {
