/**
 * Strict JSON request-body reading for the workforce-auth HTTP service
 * (IMP-010).
 *
 ...
 * control characters. Never accepts an email, password, TOTP, or backup code
 * from a query string.
 */
import "server-only";
  ...
  return new Promise<ReadJsonObjectBodyResult>((resolve) => {
    ...
    req.on("data", (chunk: Buffer) => {
      ...
      if (totalBytes > MAX_JSON_BODY_BYTES) {
        // Reject without destroying the socket: destroying here races with
        // sendJson and surfaces as a client-side fetch failure instead of
        // the intended 400 INVALID_REQUEST response.
 ...
 * sensitive fields must only ever travel in a JSON body, never a URL that
 * can end up in access logs, browser history, or a `Referer` header.
 */
