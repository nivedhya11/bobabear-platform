...
export function OrderingCatalogClient(props: { brandId: string }) {
  ...
  async function addWithStaleCartRecovery(input: Parameters<typeof addCartLine>[0]): Promise<Awaited<ReturnType<typeof addCartLine>>> {
    const result = await addCartLine(input);
    if (result.ok || (result.code !== "CART_EXPIRED" && result.code !== "CART_NOT_FOUND")) return result;
    recoverStaleGuestCart();
  ...
  async function addItem(item: CustomerMenuItem): Promise<void> {
    await withPending(item.variantId, async () => {
      ...
      if (!result.ok) {
        if (result.code === "CART_EXPIRED" || result.code === "CART_NOT_FOUND") { recoverStaleGuestCart(); return; }
        setError(commerceErrorCopy(result.code));
        return;
      }
      await updateCartFromMutation(result.data.cart);
  ...
  async function addConfiguredItem(
    ...
    try {
      ...
      if (!result.ok) {
        if (result.code === "CART_EXPIRED" || result.code === "CART_NOT_FOUND") { recoverStaleGuestCart(); setCustomizingItem(null); return; }
        setError(commerceErrorCopy(result.code));
        return;
      }
      await updateCartFromMutation(result.data.cart);
  ...
  async function saveLiveCartEdit(
    ...
    try {
      ...
      if (!result.ok) {
        if (result.code === "CART_EXPIRED" || result.code === "CART_NOT_FOUND") { recoverStaleGuestCart(); setEditTarget(null); return; }
          ...
          <div data-testid="desktop-menu" className="flex flex-col gap-6 min-w-0">
            {displayGroups.map((group) => (
              <section key={group.id} aria-label={group.name} className="flex flex-col gap-4">
                {normalizedSearch ? <h2 className="font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--text-tertiary)]">{group.name}</h2> : null}
            ...
            {normalizedSearch && displayGroups.length === 0 ? <p className="font-body text-[15px] text-[var(--text-secondary)]">Nothing in the menu matched your search.</p> : null}
          </div>
