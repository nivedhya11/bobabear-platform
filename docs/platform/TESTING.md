...
the activated `currentProductSlice` (`IMP036H_ACTIVATED: YES`; formal lifecycle
`IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE`; Product
...
`AUTHORIZED` / `STARTED` / `COMPLETE` at tip GTM-R146 / STATE-R144; `IMP036H_IMPLEMENTATION_COMPLETE: YES`;
`IMP036H_ACCEPTED: NO`; `IMP036H_FOUNDER_UAT: NOT_PERFORMED`; `pendingAcceptance` = IMP-036H;
`nextProductSlice` = IMP-036I; `IMP036I_ACTIVATED: NO`). IMP-038 remains held (`IMP038_HOLD: YES`;
