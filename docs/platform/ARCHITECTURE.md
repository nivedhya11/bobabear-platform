...
Static Next.js export → Nginx
  ...
  /api/admin/v1/*       → Administration API (dedicated workforce-admin façade on same operations process; D-373)
```

The Operations Console API boundary is architecture-locked for IMP-029 (D-372) and is implemented
in the accepted `operations` process. The Administration API boundary is architecture-locked for
IMP-035 (D-373) on the **same** `operations` process / port — no additional deployable service.
...
Fulfilment timing is orthogonal to fulfilment mode (ARCH-R23 / D-379 / ARCH-G29):

```text
FULFILMENT_MODE = DELIVERY | PICKUP    (D-378 / ARCH-G28)
FULFILMENT_TIMING = ASAP | SCHEDULED   (D-379 / ARCH-G29)
```

PICKUP still creates no Delivery aggregate. Scheduled timing does not introduce `ScheduledOrder`.
...
| ARCH-G28 | Checkout fulfilment mode (D-378, AMENDED by D-379 only for ASAP-only / no-scheduled-schema clauses): Checkout Snapshot owns the immutable fulfilment-mode commitment for a purchased order. `DELIVERY` and `PICKUP` are mutually exclusive modes, orthogonal to `FULFILMENT_TIMING` (ARCH-G29 / D-379). DELIVERY requires delivery destination/serviceability and may create Delivery execution. PICKUP binds an eligible selected Outlet, requires no customer delivery destination/serviceability, and must never create or invoke a Delivery aggregate. Mutable Checkout owns pre-payment mode intent; Order does not duplicate mutable fulfilment-mode authority; no PickupOrder. |
| ARCH-G29 | Scheduled fulfilment timing (D-379): Checkout owns mutable pre-payment fulfilment timing intent. Checkout Snapshot owns immutable purchased fulfilment timing truth. `FULFILMENT_TIMING = ASAP \| SCHEDULED`. For SCHEDULED, the Snapshot seals scheduled window start, scheduled window end, the Outlet timezone identity used for that promise, and the effective cancellation-cutoff minutes. Order references/projects Snapshot timing. No ScheduledOrder, PickupScheduledOrder, or DeliveryScheduledOrder. No new Order statuses SCHEDULED, READY, DUE, LATE, or OVERDUE. Timing is orthogonal to `FULFILMENT_MODE = DELIVERY \| PICKUP` (ARCH-G28 / D-378). |
| ARCH-G30 | Delivery successful-completion finality (D-380): `Delivery.status = DELIVERED` is durable successful Delivery execution truth. A normal replacement Delivery cannot supersede it. Correction of an incorrect committed `DELIVERED` requires separately authorized, history-preserving authority and is outside IMP-036I. Consumers such as Scheduled reminder eligibility may rely on committed `DELIVERED` completion truth without waiting for Order fulfil coordination, notification processing, or notification attempt rows. No current-Delivery pointer or lineage-tip authority is introduced. Failed or cancelled replacement remains only where already permitted by existing accepted Delivery authority and operational prerequisites. |
...
| Checkout fulfilment mode + Pickup boundary | [D-378](./decision-register.md) (AMENDED by D-379 for ASAP-only / no-scheduled-schema clauses only); ADR-018 (AMENDED; accepted mode remainder preserved); ARCH-G28 (CURRENT mode axis, orthogonal to ARCH-G29 timing); capability lock [`capabilities/IMP-036H-customer-pickup-takeaway.md`](./capabilities/IMP-036H-customer-pickup-takeaway.md) |
| Scheduled fulfilment timing + future execution boundary | [D-379](./decision-register.md) (CURRENT); ADR-019 (Accepted); ARCH-G29; capability lock [`capabilities/IMP-036I-scheduled-fulfilment.md`](./capabilities/IMP-036I-scheduled-fulfilment.md); implementation AUTHORIZED / NOT_STARTED |
| Delivery successful-completion finality | [D-380](./decision-register.md) (CURRENT); ADR-020 (Accepted); ARCH-G30; normal replacement after `DELIVERED` prohibited; runtime `createDelivery` acceptance of a `DELIVERED` predecessor is `PRE_EXISTING_DELIVERY_CONFORMANCE_DEBT` and is not fixed by this lock |
| Provider-Neutral Delivery Foundation | ARCH-R18 / ARCH-G24; capability architecture [`capabilities/IMP-031-provider-neutral-delivery-foundation.md`](./capabilities/IMP-031-provider-neutral-delivery-foundation.md) (`ARCHITECTURE_LOCKED`; implementation AUTHORIZED / STARTED / COMPLETE / COMPLETE_AND_ACCEPTED); Delivery create/request gated by ARCH-G28 / D-378 when Snapshot `fulfilmentMode = PICKUP`; successful-completion replacement interpretation clarified by D-380 / ARCH-G30 (`DELIVERED` is not a normal replacement predecessor) |
