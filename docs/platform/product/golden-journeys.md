...
| `GJ-PRODUCT-MENU-LAUNCH` / `JOURNEY-PRODUCT-MENU-LAUNCH` — Product/menu launch | `PERSONA-WORKFORCE-OPERATOR`, `PERSONA-CUSTOMER` | `CURRENT` | Bounded commercial configuration → Menu organization and outlet context → customer discovery/orderability continuity accepted through IMP-036F `COMPLETE_AND_ACCEPTED` (GTM-R122 / STATE-R120). Covers the accepted Brand commercial launch path within existing Catalog/Menu/Assortment/Pricing/Promotions authorities; no new media-upload platform, invented promotion lifecycle, or bulk semantics beyond the accepted F slice. | [IMP-036F capability](../capabilities/IMP-036F-catalog-menu-pricing-promotions-management.md); [IMP-036F Product Definition](./IMP-036F/product-definition.md); [ROADMAP](../ROADMAP.md) / [STATE](../STATE.md). |
