...
| E | `npm run test:scripts` — `audit-persistence.test.mjs` | File-level exit 1 despite helper subtests | Allowlist must cover current admin/ops/workforce-auth, IMP-036C fixtures, and legitimate platform readiness consumer; public app/component persistence boundary remains strict (including type-only imports); direct-execution main-guard must not set `exitCode` on import | `AUDIT_TOOLING_DEFECT` | Stale allowlist consumers added where legitimate (incl. `src/platform/observability/health.ts`); direct-execution/main-guard repaired; type-only exemption removed — `src/app/**` and `src/components/**` persistence imports remain prohibited; negative regression coverage proves ordinary and type-only rejects | PASS |
