...
| E | `npm run test:scripts` — `audit-persistence.test.mjs` | File-level exit 1 despite helper subtests | Allowlist must cover current admin/ops/workforce-auth and IMP-036C fixtures; public app/component persistence boundary remains strict (including type-only imports and re-exports); direct-execution main-guard must not set `exitCode` on import | `AUDIT_TOOLING_DEFECT` | Stale allowlist consumers added where legitimate; health.ts retains its existing injected-Persistence design and may import only the Persistence type from the dedicated persistence types module; runtime persistence imports/re-exports remain prohibited; direct-execution/main-guard repaired; type-only does not bypass `src/app/**` / `src/components/**`; negative regressions cover imports and re-exports | PASS |
