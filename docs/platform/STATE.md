...
STATE-R111 records IMP-036E implementation complete pending independent acceptance after exact
implementation merge (`IMP036E_IMPLEMENTATION_MERGE_SHA:
0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739`; `IMP036E_IMPLEMENTATION_TREE:
0def59ce9bcab4575a7b43b7c3c07c85ac575428`), independent implementation review PASS on reviewed
product candidate (`IMP036E_REVIEWED_CANDIDATE_HEAD:
b2ffaa5bc2b5c6f58ff5241c226b583160132837`; `IMP036E_REVIEWED_CANDIDATE_TREE:
0def59ce9bcab4575a7b43b7c3c07c85ac575428`), and successful post-merge CI (`workflow: CI`; run
`34055991330`; head_sha `0ebb5e937cd7ac14bb3e39e9d1d494e32c9d2739`; result SUCCESS). Implementation
evidence is **COMPLETE** (`IMP036E_IMPLEMENTATION_EVIDENCE: COMPLETE`); independent implementation
review is **PASS** (`IMP_036E_INDEPENDENT_IMPLEMENTATION_REVIEW: PASS`). IMP-036E lifecycle is
`IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE`. Architecture remains `ARCHITECTURE_LOCKED`
(`IMP-036E_ARCHITECTURE_LOCKED: YES`); implementation is `AUTHORIZED` / `STARTED` / `COMPLETE`
(`IMP-036E_IMPLEMENTATION: AUTHORIZED / STARTED / COMPLETE`;
`IMP-036E_IMPLEMENTATION_AUTHORIZED: YES`; `IMP-036E_STARTED: YES`;
`IMP-036E_IMPLEMENTATION_COMPLETE: YES`; `IMP-036E_ACCEPTED: NO`). Completion is **not** acceptance.
Locked capability architecture remains at
[`capabilities/IMP-036E-store-operations-management.md`](./capabilities/IMP-036E-store-operations-management.md).
`acceptedThrough` remains IMP-036D; `pendingAcceptance` becomes IMP-036E; `currentProductSlice`
remains IMP-036E; `nextProductSlice` remains IMP-036F, which remains `PLANNED` / `NOT_ACTIVATED` /
`NOT_AUTHORIZED` / `NOT_STARTED`. Current Product Implementation remains IMP-036E — Store
Operations Management. Founder Option A Assortment remains Brand authority
...
`ARCH_R20_CREATED: NO`. ARCH-R19 and DR-15 remain unchanged. Founder UAT remains required and has
**not** been performed (`IMP-036E_FOUNDER_UAT_REQUIRED: YES`; `IMP-036E_FOUNDER_UAT: NOT_STARTED`).
GitHub Pages is not Founder staging/UAT evidence (`FOUNDER_STAGING_DEPLOYMENT: NOT_PERFORMED`;
`FOUNDER_UAT: NOT_PERFORMED`). Does **not** activate IMP-036F. IMP-036D remains
`COMPLETE_AND_ACCEPTED` and is not reopened. Supersedes STATE-R110 for the current product-slice
implementation-completion position. Historical STATE-R110 implementation-start checkpoint remains
preserved as prior authority.

STATE-R110 records the separate Founder implementation-start gate for **IMP-036E — Store
Operations Management** under the locked capability architecture and prior STATE-R109
authorization. IMP-036E lifecycle is `IMPLEMENTATION_IN_PROGRESS`. Architecture remains
`ARCHITECTURE_LOCKED` (`IMP-036E_ARCHITECTURE_LOCKED: YES`); implementation is `AUTHORIZED` /
`STARTED` (`IMP-036E_IMPLEMENTATION: AUTHORIZED / STARTED`;
`IMP-036E_IMPLEMENTATION_AUTHORIZED: YES`; `IMP-036E_STARTED: YES`;
`IMP-036E_IMPLEMENTATION_COMPLETE: NO`; `IMP-036E_ACCEPTED: NO`). Start does **not** complete or
...
`NOT_AUTHORIZED` / `NOT_STARTED`. Current Product Implementation is IMP-036E — Store Operations
Management. Founder Option A Assortment remains Brand authority
(`IMP036E_ASSORTMENT_AUTHORITY: BRAND`; `OUTLET_MANAGER_OUTLET_SCOPE_ASSORTMENT_MANAGE: NO`;
`OUTLET_EFFECTIVE_ASSORTMENT_PRESENTATION: AUTHORIZED_READ_OR_ESCALATE`;
`IMP036E_ASSORTMENT_WORKFORCE_TRANSPORT: READ_ONLY_OPERATIONS_PROJECTION`;
`ASSORTMENT_AUTHORIZATION_RESOURCE: BRAND_DERIVED_FROM_OUTLET`;
...
start checkpoint. Does **not** activate IMP-036F. IMP-036D remains `COMPLETE_AND_ACCEPTED` and is
not reopened. Supersedes STATE-R109 for the current product-slice implementation-start position.
Historical STATE-R109 implementation-authorization checkpoint remains preserved as prior authority.
...
`ARCHITECTURE_LOCKED` (`IMP-036D_ARCHITECTURE_LOCKED: YES`); implementation is `AUTHORIZED` /
`NOT_STARTED` (`IMP-036D_IMPLEMENTATION_AUTHORIZED: YES`; `IMP-036D_STARTED: NO`;
`IMP-036D_IMPLEMENTATION_COMPLETE: NO`; `IMP-036D_ACCEPTED: NO`). Authorization does **not**
auto-start implementation and does **not** equal `IMPLEMENTATION_IN_PROGRESS`. Locked capability
architecture remains at
[`capabilities/IMP-036D-workforce-franchise-operations-v2.md`](./capabilities/IMP-036D-workforce-franchise-operations-v2.md).
`acceptedThrough` remains IMP-036C; `pendingAcceptance` remains NONE; `currentProductSlice` remains
...
`IMP036D_FINANCIAL_DOCUMENT_WORKFORCE_REVIEW: DEFERRED`;
`IMP036D_NOTIFICATION_RESEND_WORKFORCE_TRANSPORT: APPROVED_FOR_ARCHITECTURE`. Founder UAT remains
required eventually (`IMP-036D_FOUNDER_UAT_REQUIRED: YES`). No product implementation accompanies
this authorization. Supersedes STATE-R102 for the current product-slice
implementation-authorization position.
...
`IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE` (`IMP-036C_ACCEPTED: NO`). Locked boundary facts:
`schema_change: YES` bounded to `0036_outlet_delivery_fee_policy` on
`outlet_serviceability_configs`; `provider_IO: NO`; `new_service: NO`; `new_queue: NO`;
`new_auth_model: NO`; `new_roles: NO`; `new_permissions: NO`;
`STANDARDIZED_CUSTOMER_DELIVERY_FEE: YES`. `acceptedThrough` remains IMP-036B;
`currentProductSlice` and `pendingAcceptance` become IMP-036C; `nextProductSlice` becomes IMP-036D,
which remains `PLANNED` / `NOT_ACTIVATED`. Founder UAT is **required**
(`IMP-036C_FOUNDER_UAT_REQUIRED: YES`). Completion is not acceptance and does not authorize IMP-036D.
Supersedes STATE-R98 for the current IMP-036C lifecycle position.

STATE-R98 records formal acceptance of **IMP-036B — Customer Account, Onboarding, Address & Location
...
`4c4fcf1887fa6d8386575c77d5da22bb11e79059` (merge PR #71). Implementation evidence is **COMPLETE**;
independent implementation review is **PASS**; independent acceptance evidence is **ACCEPTED**;
Founder UAT is **PASS**; formal acceptance is recorded (`IMP036B_FORMAL_ACCEPTANCE: ACCEPTED`;
`IMP-036B_ACCEPTED: YES`). IMP-036B is `COMPLETE_AND_ACCEPTED`; architecture remains `LOCKED`
(`IMP-036B_ARCHITECTURE_LOCKED: YES`). `acceptedThrough` advances to IMP-036B;
`currentProductSlice` and `pendingAcceptance` become NONE; `nextProductSlice` remains IMP-036C,
which remains `PLANNED` / `NOT_ACTIVATED` until STATE-R99. Formal acceptance does **not** authorize,
start, or activate IMP-036C. Supersedes STATE-R97 for the current IMP-036B lifecycle and acceptance
position.
...
IMP-033 is `COMPLETE_AND_ACCEPTED`; architecture remains `LOCKED`
(`IMP-033_ARCHITECTURE_LOCKED: YES`); implementation remains `AUTHORIZED` / `STARTED` / `COMPLETE`
(`IMP-033_IMPLEMENTATION_AUTHORIZED: YES`; `IMP-033_STARTED: YES`;
`IMP-033_IMPLEMENTATION_COMPLETE: YES`). Locked boundary facts remain unchanged: `schema_change: YES`
bounded to Notifications-owned tables and migrations, `provider_IO: NO`, `new_service: NO`, and
`async_topology: POSTGRESQL_TRANSACTIONAL_OUTBOX_IN_PROCESS_WORKER`. `acceptedThrough` advances to
IMP-033; `currentProductSlice` and `pendingAcceptance` are NONE; `nextProductSlice` remains IMP-034 —
Meta WhatsApp Cloud API Adapter, `PLANNED` / `NOT_ACTIVATED` / `NOT_AUTHORIZED` / `NOT_STARTED`.
Formal acceptance does **not** authorize, start, or activate IMP-034, select a WhatsApp BSP, perform
Meta production onboarding, or add provider webhook routes. Founder UAT is not required for this
foundation-only slice (`IMP-033_FOUNDER_UAT_REQUIRED: NO`; `IMP-033_FOUNDER_UAT: NOT_APPLICABLE`) and
is not claimed as PASS; independent technical acceptance was the applicable gate. ARCH-R18 /
ARCH-G24 and DR-14 remain unchanged; `D-373` is absent. This reconciliation introduces no product
source, schema, migration, decision-register, or global-architecture substance mutation. Supersedes
STATE-R86 for the current IMP-033 lifecycle and acceptance position.

STATE-R86 records a single founder-authorized **combined** gate for **IMP-033 — Notification
Foundation**: capability-local architecture lock, implementation authorization, implementation start,
and implementation completion, with formal acceptance **not** claimed. Locked capability
architecture is persisted at
[`capabilities/IMP-033-notification-foundation.md`](./capabilities/IMP-033-notification-foundation.md)
under binding ADR-012. Lifecycle becomes `IMPLEMENTATION_COMPLETE_PENDING_ACCEPTANCE`; architecture
is `ARCHITECTURE_LOCKED` (`IMP-033_ARCHITECTURE_LOCKED: YES`); implementation is `AUTHORIZED` /
`STARTED` / `COMPLETE` (`IMP-033_IMPLEMENTATION_AUTHORIZED: YES`; `IMP-033_STARTED: YES`;
`IMP-033_IMPLEMENTATION_COMPLETE: YES`; `IMP-033_ACCEPTED: NO`). `acceptedThrough` remains IMP-032;
`pendingAcceptance` becomes IMP-033; `currentProductSlice` remains IMP-033; and `nextProductSlice`
remains IMP-034 — Meta WhatsApp Cloud API Adapter, which remains `PLANNED` / `NOT_ACTIVATED`.
Founder UAT is **not required** (`IMP-033_FOUNDER_UAT_REQUIRED: NO`); IMP-033 is a foundation-only
slice with no customer-visible or operator-visible interactive surface, on the same basis as
IMP-031. Locked boundary facts: `schema_change: YES` bounded to Notifications-owned tables and
migrations only (`0033_notifications`); `provider_IO: NO` — no Meta/WhatsApp provider I/O, channel
adapters are ports/foundation only, shipped adapters are explicitly non-sending, and IMP-033 must
never fabricate `PROVIDER_ACCEPTED` / `DELIVERED` / `READ`, provider identifiers, or external-send
success; `new_service: NO`; and
`async_topology: POSTGRESQL_TRANSACTIONAL_OUTBOX_IN_PROCESS_WORKER` using the existing PostgreSQL
transactional outbox and existing modular-monolith in-process worker conventions
(`NotificationOutboxProcessor` hosted in customer-commerce and operations like
`PaymentInboxProcessor`), with no Redis / Kafka / RabbitMQ / external queue / new deployable
service. Operator resend is authorized by the single new `notification.resend` permission on the
existing `support_refund_operator` role (plus `platform_super_admin` via all-permissions); no new
role is invented. The Meta WhatsApp Cloud API adapter, webhook routes, Meta onboarding, conversation
console UI, and marketing automation remain deferred to IMP-034 and later slices. `D-373` is not
created; `ARCH_R19_REQUIRED: NO`; ARCH-R18 and DR-14 remain unchanged. Completion does **not** equal
acceptance. Supersedes STATE-R85 for the current IMP-033 lifecycle position. Product acceptance
through IMP-032 is unchanged.
...
activation position. The historical STATE-R85 activation checkpoint remains preserved as prior
authority; STATE-R86 supersedes it.
...
[`capabilities/IMP-032-dehradun-delivery-operating-mode.md`](./capabilities/IMP-032-dehradun-delivery-operating-mode.md)
(`ARCHITECTURE_LOCKED`; `IMP-032_ARCHITECTURE_LOCKED: YES`). Operating mode remains
**MANUAL_PROVIDER_NEUTRAL_DEHRADUN_DELIVERY**. Lifecycle is `IMPLEMENTATION_IN_PROGRESS`.
Implementation is `AUTHORIZED` / `STARTED` (`IMP-032_IMPLEMENTATION_AUTHORIZED: YES`;
`IMP-032_STARTED: YES`; `IMP-032_IMPLEMENTATION_COMPLETE: NO`; `IMP-032_ACCEPTED: NO`). Start does
**not** complete or accept implementation. `acceptedThrough` remains IMP-031; `pendingAcceptance`
remains NONE; `currentProductSlice` remains IMP-032; and `nextProductSlice` remains IMP-033 —
Notification Foundation, which remains `PLANNED` / `NOT_ACTIVATED`. IMP-031 remains
`COMPLETE_AND_ACCEPTED` under ARCH-R18 / ARCH-G24. No named provider is canonical; no provider API,
webhook, queue, worker, notification, Delivery schema migration, runtime topology, D-373, or
ARCH-R19 is introduced. ARCH-R18 and DR-14 remain unchanged; `D-373_CREATED: NO`;
`ARCH_R19_REQUIRED: NO`. Founder UAT remains required for later acceptance. Supersedes STATE-R80
for the current IMP-032 implementation-start position. Historical STATE-R80 authorization
checkpoint remains preserved as prior authority.
...
[`capabilities/IMP-032-dehradun-delivery-operating-mode.md`](./capabilities/IMP-032-dehradun-delivery-operating-mode.md)
(`ARCHITECTURE_LOCKED`; `IMP-032_ARCHITECTURE_LOCKED: YES`). Operating mode
**MANUAL_PROVIDER_NEUTRAL_DEHRADUN_DELIVERY** is locked: operator-approved dispatch;
pre-external-attempt `REQUESTED` → `BOOKING_OUTCOME_UNKNOWN`; stable `bookingCorrelationId`;
application-level manual booking resolution with no provider I/O; BOBA order page as canonical
customer delivery-status surface; deferred provider API / webhook / worker / queue / WhatsApp
automation. Architecture lock does **not** authorize or start implementation
(`IMP-032_IMPLEMENTATION_AUTHORIZED: NO`; `IMP-032_STARTED: NO`;
`IMP-032_IMPLEMENTATION_COMPLETE: NO`; `IMP-032_ACCEPTED: NO`). `acceptedThrough` remains IMP-031;
`pendingAcceptance` remains NONE; `currentProductSlice` remains IMP-032; and `nextProductSlice`
remains IMP-033 — Notification Foundation, which remains `PLANNED` / `NOT_ACTIVATED`. IMP-031 remains
`COMPLETE_AND_ACCEPTED` under ARCH-R18 / ARCH-G24. No named provider is canonical; no provider API,
webhook, queue, worker, notification, schema migration, runtime topology, D-373, or ARCH-R19 is
introduced. ARCH-R18 and DR-14 remain unchanged; `D-373_CREATED: NO`; `D373_REQUIRED_FOR_LOCK: NO`;
`ARCH_R19_REQUIRED: NO`. Reuses the uncommitted STATE-R79 revision for architecture lock (draft was
never committed or promoted). Supersedes STATE-R78 for the current IMP-032 architecture-lock
position.

STATE-R78 records explicit authorization and canonical activation of **IMP-032 — Dehradun Delivery
Operating Mode** for architecture work only. IMP-032 is `ARCHITECTURE_IN_PROGRESS`; architecture is
`NOT_LOCKED` (`IMP-032_ARCHITECTURE_LOCKED: NO`); implementation is `NOT_AUTHORIZED` / `NOT_STARTED`
(`IMP-032_IMPLEMENTATION_AUTHORIZED: NO`; `IMP-032_STARTED: NO`;
`IMP-032_IMPLEMENTATION_COMPLETE: NO`; `IMP-032_ACCEPTED: NO`). `acceptedThrough` remains IMP-031;
`pendingAcceptance` remains NONE; `currentProductSlice` becomes IMP-032; and `nextProductSlice`
becomes IMP-033 — Notification Foundation, which remains `PLANNED` / `NOT_ACTIVATED`. IMP-031 remains
`COMPLETE_AND_ACCEPTED` under ARCH-R18 / ARCH-G24 and the locked capability artifact at
[`capabilities/IMP-031-provider-neutral-delivery-foundation.md`](./capabilities/IMP-031-provider-neutral-delivery-foundation.md).
Provider-neutral Delivery authority, Order lifecycle separation, provider-observation evidence
semantics, one-active Delivery/booking safety, `BOOKING_OUTCOME_UNKNOWN` semantics, customer delivery
charge versus provider cost separation, and deferral of provider-specific choices remain binding and
are not amended. No capability architecture artifact is created, architecture is not locked,
implementation is not authorized or started, no provider or aggregator is selected, no Dehradun
operating mode is chosen, and no runtime, schema, migration, dependency, deployment,
decision-register, or global-architecture substance mutation is introduced. ARCH-R18 and DR-14 remain
unchanged; `D-373_CREATED: NO`; `NO_NEW_CURRENT_DECISION_IN_THIS_ACTIVATION_GATE: YES`. Subsequent
architecture discovery may investigate operating-model and provider-strategy questions without this
gate answering them. Supersedes STATE-R77 for the current product-slice position.
...
[`capabilities/IMP-031-provider-neutral-delivery-foundation.md`](./capabilities/IMP-031-provider-neutral-delivery-foundation.md).
Authorization scope remains locked Boundary C only. ARCH-R18 / ARCH-G24 remain binding. IMP-031 is
`IMPLEMENTATION_IN_PROGRESS`; architecture remains `LOCKED` (`IMP-031_ARCHITECTURE_LOCKED: YES`);
implementation is `AUTHORIZED` / `STARTED` (`IMP-031_IMPLEMENTATION_AUTHORIZED: YES`;
`IMP-031_STARTED: YES`). This is implementation start only; implementation is not complete or
accepted. `acceptedThrough` remains IMP-030; `pendingAcceptance` remains NONE; `currentProductSlice`
remains IMP-031; and `nextProductSlice` remains IMP-032. DR-14 is unchanged; D-373 is absent.
Concrete provider adapters, provider selection/accounts/credentials, Dehradun operating mode,
provider payload/status mapping, concrete callback/webhook API, workers/queues/new services, retry
...
introduced by this governance START gate. Supersedes STATE-R74 for the current IMP-031
implementation-start position.
...
at
[`capabilities/IMP-031-provider-neutral-delivery-foundation.md`](./capabilities/IMP-031-provider-neutral-delivery-foundation.md).
ARCH-R18 / ARCH-G24 and implementation boundary C with the capability-local Delivery lifecycle
amendment remain binding. IMP-031 is `ARCHITECTURE_LOCKED`; architecture is `LOCKED`
...
IMP-030; `pendingAcceptance` remains NONE; `currentProductSlice` remains IMP-031; and
`nextProductSlice` remains IMP-032. DR-14 is unchanged; D-373 is absent. No provider, operating
mode, route, runtime topology, schema, migration, dependency, credential, implementation,
or deployment is selected or introduced. This is an architecture-lock-only transition. Supersedes
STATE-R72 for the current IMP-031 architecture-lock position.

STATE-R72 records the reviewable architecture candidate for **IMP-031 — Provider-Neutral Delivery
Foundation** at
[`capabilities/IMP-031-provider-neutral-delivery-foundation.md`](./capabilities/IMP-031-provider-neutral-delivery-foundation.md)
and advances global architecture to **ARCH-R18 / ARCH-G24** for minimal durable provider-neutral
Delivery authority. Implementation boundary C is approved with the capability-local Delivery
lifecycle amendment. IMP-031 remains
`ARCHITECTURE_IN_PROGRESS`; architecture remains `NOT_LOCKED`; implementation remains
`NOT_AUTHORIZED` / `NOT_STARTED` (`IMP-031_IMPLEMENTATION_AUTHORIZED: NO`; `IMP-031_STARTED: NO`).
`acceptedThrough` remains IMP-030; `pendingAcceptance` remains NONE; `currentProductSlice` remains
IMP-031; and `nextProductSlice` remains IMP-032. DR-14 is unchanged; D-373 is absent. No provider,
operating mode, route, runtime topology, schema, migration, dependency, credential, implementation,
or deployment is selected or introduced. Supersedes STATE-R71 for the current IMP-031
architecture-drafting position.
...
| IMP-032 locked capability architecture | [`capabilities/IMP-032-dehradun-delivery-operating-mode.md`](./capabilities/IMP-032-dehradun-delivery-operating-mode.md) (`ARCHITECTURE_LOCKED`; implementation AUTHORIZED / STARTED) |
