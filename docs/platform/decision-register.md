...
- New decisions continue after the highest CURRENT/AMENDED/PROPOSED register ID: next ID **D-381**
  (**D-379** is **CURRENT** for Scheduled Fulfilment Timing + Future Execution Boundary; **D-380**
  is **CURRENT** for Delivery Successful-Completion Finality + Replacement Boundary).
...
| D-371 | Durable Cart Unit Sequence Authority: Cart retains the existing coalesced configured-line model: configuration equality remains line identity and quantity is excluded from it. Every active Cart-line unit has one durable internal unit-sequence record identifying its Cart, coalesced Cart line, base `variantId` (directly or derivably), and immutable server-issued monotonically orderable add ordinal. `COUNT(active unit records for a line) = line.quantity` MUST hold transactionally. Every successful single-unit add resolves current D-369 configuration authority, resolves/creates its coalesced line, increments that line, and appends exactly one unit record under existing Cart lock/revision authority. A product-level decrement takes Cart authority, `variantId`, and normal concurrency input; the server, never the browser, atomically selects the newest active unit for that `variantId`, decrements its line, consumes that record, removes a zero line, advances normal Cart state, and returns the standard Cart projection. Specific-line increment appends a record; specific-line decrement consumes that line's newest record; line removal consumes all its records; Cart clear, expiry, and deletion remove associated records under existing lifecycle conventions. D-370 identity transition must atomically preserve immutable unit ordinals while moving records to the surviving Cart/line; reconciled equivalent lines retain their source records and no history is renumbered. Existing Cart row locks/revision authority govern adds and removals; there is no second concurrency model. Unit sequence remains internal; card aggregate is a projection of Cart quantities. Forward-only migration is required. Historical order in a pre-D-371 active Cart is unknowable and MUST NOT be fabricated from UUIDs, timestamps, display order, revision, or browser state; authoritative FILO/LIFO begins only for Cart state established under the new invariant. This decision authorizes the minimum existing customer-commerce `/api/v1/*` Cart command/transport needed for product-level latest-unit decrement, not a new service, aggregate, event platform, broker, dynamic Next.js execution, or topology. It does not change D-368 Menu read authority, D-369 paid-modifier explicit-selection authority, D-370 identity-transition policy, pricing, Checkout Snapshot final truth, Payment, Order, Refund, financial documents, auth, static export, or Nginx topology; implementation remains separately governed by the amended IMP-028D RC3 capability contract | Customer Commerce / Cart / IMP-028D RC3 | CURRENT | This register + [`ARCHITECTURE.md`](./ARCHITECTURE.md) + [`capabilities/IMP-028D-desktop-ordering-continuity.md`](./capabilities/IMP-028D-desktop-ordering-continuity.md) | Competing product-card removal authority based on client stack/order, Cart-line UUID/display/update order, price/title, or non-durable state | — | ARCH-G11, ARCH-G22, D-369, D-370, existing Cart configured-line identity / revision concurrency |
...
| D-377 | Pre-GTM Product Insertion + GTM Infrastructure Program Pause: Founder-authorized insertion of **IMP-036H** (Customer Pickup / Takeaway) as `currentProductSlice` with `PD-IMP-036H-DRAFT-1` = `DRAFT_READY_FOR_GATE` (`IMP036H_ACTIVATED: YES`; Gate / Fit / lock / implementation authorization **not** performed). Reserves **IMP-036I** Scheduled Fulfilment as `PLANNED` only (`IMP036I_ACTIVATED: NO`; `nextProductSlice`). Establishes `PROGRAM_PAUSE: PRE_GTM_PRODUCT_INSERTION_PROVIDER_BLOCKED`. Holds **IMP-037** (`IMP037_HOLD: YES`; `IMPLEMENTATION_IN_PROGRESS`; `PHASE1_BLOCK_STATUS: BLOCKED_PROVIDER_ACCESS`; `IMP037_IMPLEMENTATION_COMPLETE: NO`; `IMP037_ACCEPTED: NO`) and **IMP-038** (`IMP038_HOLD: YES`; `IMPLEMENTATION_COMPLETE` / `NOT_ACCEPTED`; `IMP038_EXTERNAL_ASSESSMENT: DEFERRED_UNTIL_PRE_GTM_APPLICATION_SCOPE_STABILIZES`; frozen runtime evidence preserved; `GAP-EXT-ASSESS-001: NOT_CLOSED`). Does **not** activate IMP-039/040; does **not** accept IMP-037/038; does **not** authorize IMP-036H implementation; does **not** create an ARCH revision. Preserves historical `CONTINUATION_EXCEPTION: IMP037_PROVIDER_BLOCKED_TO_IMP038` as history; does **not** reopen IMP-026→028 exception; does **not** reopen IMP-038 implementation. | Roadmap / Program / IMP-036H / IMP-036I / IMP-037 / IMP-038 | CURRENT | This register + [`ROADMAP.md`](./ROADMAP.md) + [`STATE.md`](./STATE.md) + [`product/IMP-036H/product-definition.md`](./product/IMP-036H/product-definition.md) | — | — | GTM-R141 / STATE-R139 activation; prior tip GTM-R142 / STATE-R140 Gate PASS; CURRENT tip GTM-R143 / STATE-R141 Architecture Fit PASS + lock; PROGRAM_PAUSE; IMP-036H Product Definition; IMP-037/038 HOLD |
| D-378 | Checkout Fulfilment Mode + Pickup Execution Boundary: Checkout Snapshot owns immutable fulfilment commitment (`DELIVERY` \| `PICKUP`); DELIVERY requires destination/serviceability and may create Delivery execution; PICKUP binds eligible selected Outlet, requires no customer delivery destination/serviceability, never creates/invokes Delivery; mutable Checkout owns pre-payment mode + `pickupOutletId`; commercial evaluation must not apply delivery charges for PICKUP; OutletPickupProfile is bounded mutable outlet configuration under existing `outlet.read`/`outlet.update`; no new Order lifecycle, payment model, role, permission, deployable service, or scheduled-fulfilment schema (IMP-036I). Financial Document adapters remain D-365 compliant (corrected Option A): Pickup does **not** invent a Delivery destination; does **not** classify Pickup location as recipient/customer address; does **not** reconstruct recipient facts from mutable customer state; optional Pickup recipient fields remain absent (`null`) unless immutable accepted recipient authority already exists. Place-of-supply remains under existing D-365 issuer/profile policy. No new legal claim. Binding ARCH-G28 / ARCH-R22 / ADR-018. Does **not** authorize IMP-036H implementation. | Checkout / Fulfilment / Pickup / Delivery boundary / IMP-036H | AMENDED | This register + [`ARCHITECTURE.md`](./ARCHITECTURE.md) + [`decisions/ADR-018-customer-fulfilment-mode-and-pickup-boundary.md`](./decisions/ADR-018-customer-fulfilment-mode-and-pickup-boundary.md) + [`capabilities/IMP-036H-customer-pickup-takeaway.md`](./capabilities/IMP-036H-customer-pickup-takeaway.md) | — | — | ARCH-G28; IMP-036H Architecture Fit PASS + lock (implementation NOT_AUTHORIZED) |
| D-379 | Scheduled Fulfilment Timing + Future Execution Boundary: Checkout owns mutable pre-payment `FULFILMENT_TIMING = ASAP \| SCHEDULED`; Checkout Snapshot owns immutable purchased timing truth (window start/end UTC, sealed IANA timezone, sealed cancellation-cutoff minutes); Order projects Snapshot; no ScheduledOrder aggregate; no new Order lifecycle statuses; no Slot capacity/reservation engine; server-authoritative eligibility over IMP-036E hours + minimal Outlet future-closure exceptions + Outlet lead-time profile; Brand cancellation policy (defaults 30/60; 0–240); pre-payment revalidation reuses `prepareCheckoutForPayment`; Scheduled Delivery preserves IMP-032 operator-approved manual dispatch; Pickup immediate Ops visibility (no prep-release state); one proactive reminder via notification-owned outbox `available_at` only (not a generic Scheduled business-action bus); no new deployable service/queue/broker/role/permission. ARCH-G29 / ARCH-R23 / ADR-019 CURRENT / Accepted. Amends D-378 only for ASAP-only / no-scheduled-schema reservation D-379 CURRENT amends those clauses only. Scheduled Delivery reminder completion consumes CURRENT **D-380** Delivery successful-completion truth (`EXISTS DELIVERED`); D-379 does **not** redefine Delivery replacement semantics. Does **not** authorize IMP-036I implementation. Architecture Fit PASS review `5312653831`. | Checkout / Snapshot / Order projection / Outlet schedule / Delivery coordination / Notification / IMP-036I | CURRENT | This register + [`ARCHITECTURE.md`](./ARCHITECTURE.md) (ARCH-R23) + [`decisions/ADR-019-scheduled-fulfilment-timing-and-execution.md`](./decisions/ADR-019-scheduled-fulfilment-timing-and-execution.md) + [`capabilities/IMP-036I-scheduled-fulfilment.md`](./capabilities/IMP-036I-scheduled-fulfilment.md); depends on **D-380** CURRENT for Delivery completion fact | — (amends **D-378** ASAP-only / scheduled-schema reservation only) | — | ARCH-G29 CURRENT; IMP-036I architecture locked; consumes D-380 CURRENT |
| D-380 | Delivery Successful-Completion Finality + Replacement Boundary: `Delivery.status = DELIVERED` is durable successful Delivery execution truth; a normal replacement Delivery MUST NOT be created after `DELIVERED`; normal replacement after `FAILED` / `CANCELLED` remains governed only by existing accepted Delivery rules and prerequisites (no eligibility expansion); incorrect committed `DELIVERED` requires a separately authorized history-preserving correction mechanism outside IMP-036I; Scheduled Delivery reminder eligibility may suppress on `EXISTS` authoritative `DELIVERED` for the exact Order without lineage-tip / `current_delivery_id` / latest-row-wins authority; current runtime `createDelivery` acceptance of a `DELIVERED` predecessor is `PRE_EXISTING_DELIVERY_CONFORMANCE_DEBT` against CURRENT D-380 and must be corrected before IMP-036I acceptance. Clarifies/amends IMP-031 “authoritatively inactive” replacement interpretation for successful-completion finality only; does **not** supersede IMP-031 wholesale, Delivery lifecycle, one-active rule, stable request identity, booking ambiguity, failure/cancellation, return progression, IMP-032 manual Dehradun mode, or Order lifecycle. ARCH-G30 / ARCH-R23 / ADR-020 CURRENT / Accepted (shared lock with D-379 / ARCH-G29). Human architecture direction APPROVED 2026-09-25; independent Architecture Fit PASS review `5312653831`. Does **not** authorize implementation. | Delivery / Fulfilment / Replacement / IMP-031 / IMP-036I | CURRENT | This register + [`ARCHITECTURE.md`](./ARCHITECTURE.md) (ARCH-R23) + [`decisions/ADR-020-delivery-successful-completion-finality.md`](./decisions/ADR-020-delivery-successful-completion-finality.md) + [`capabilities/IMP-036I-scheduled-fulfilment.md`](./capabilities/IMP-036I-scheduled-fulfilment.md) | — (clarifies/amends IMP-031 successful-completion replacement interpretation only; does not supersede IMP-031 wholesale) | — | ARCH-G30 CURRENT; Delivery DELIVERED finality; normal replacement prohibition after DELIVERED; Scheduled reminder completion authority |
...
| ADR-017 | Edge / origin trust / application-authoritative security hardening | CURRENT | Cloudflare Free supplemental edge + origin trust chain + Nginx CSP/real_ip + Turnstile challenge + abuse/step-up layering; **D-375**; CSP §8.2 Maps Fonts hosts narrowly amended by **D-376** |
| ADR-018 | Customer fulfilment mode and Pickup boundary | AMENDED | Checkout Snapshot owns immutable `DELIVERY` \| `PICKUP`; Pickup fail-closed vs Delivery; Financial Document Option A; **D-378** AMENDED by **D-379** only for ASAP-only / no-scheduled-schema clauses; accepted mode remainder preserved; ARCH-G28 |
| ADR-019 | Scheduled fulfilment timing and execution boundary | Accepted | Checkout/Snapshot timing ownership `ASAP` \| `SCHEDULED`; no Slot capacity engine; sealed cutoff; IMP-032 manual Delivery preserved; notification reminder via owned outbox; **D-379** CURRENT; reminder Delivery completion depends on **D-380** CURRENT; ARCH-G29 / ARCH-R23 |
| ADR-020 | Delivery successful-completion finality | Accepted | `DELIVERED` durable successful execution truth; no normal replacement after `DELIVERED`; FAILED/CANCELLED replacement only under existing accepted prerequisites; history-preserving correction outside IMP-036I; **D-380** CURRENT; ARCH-G30 / ARCH-R23; runtime DELIVERED-predecessor acceptance is `PRE_EXISTING_DELIVERY_CONFORMANCE_DEBT` |
...
| D-378 / ADR-018 | **D-379** / ADR-019 | Mode authority remains: `DELIVERY` \| `PICKUP`, Pickup fail-closed Delivery boundary, selected Outlet, Pickup destination/privacy, Pickup delivery-charge boundary, Financial Document Option A, no Delivery aggregate for Pickup. Amended only for ASAP-only / no-scheduled-schema clauses. Fulfilment mode is orthogonal to fulfilment timing. |
...
- Promoted **D-379** from PROPOSED → **CURRENT** (Scheduled Fulfilment Timing + Future Execution Boundary; ADR-019 Accepted; ARCH-G29).
- Promoted **D-380** from PROPOSED → **CURRENT** (Delivery Successful-Completion Finality + Replacement Boundary; ADR-020 Accepted; ARCH-G30).
...
- IMP-036I implementation start is recorded in ROADMAP GTM-R156 / STATE-R154 without a new D-number. Prior authorization remains GTM-R155 / STATE-R153. Tranche 2 eligibility and payment bind is recorded in ROADMAP GTM-R157 / STATE-R155 without a new D-number. Independent Tranche 2 verification is PASS against main `557db375545c5f0853ec894919fd1e0d6f696770` / tree `35b19c62c080c9c1b432b0dea8937c717d015b72`. Tranche 3 customer, operations, and configuration surfaces are recorded in ROADMAP GTM-R158 / STATE-R156 without a new D-number. DR-21 did not itself authorize or start implementation. Current: `IMP036I_IMPLEMENTATION_AUTHORIZED: YES`; `IMP036I_IMPLEMENTATION_STARTED: YES`; `IMP036I_IMPLEMENTATION_COMPLETE: NO`. Tranche 1 closed `PRE_EXISTING_DELIVERY_CONFORMANCE_DEBT` in runtime. Next action is independent Tranche 4 verification. Independent Tranche 3 verification is PASS against main `9b1c73b39f6d709e3e8f9adf7c772564285234e7` / tree `adea96bff22f54c6aa57d68721d658cfc24e5d73`. Tranche 4 purchased cancellation and scheduled reminder are recorded in ROADMAP GTM-R159 / STATE-R157 without a new D-number. This record does not accept IMP-036I, perform Founder UAT, or authorize production cutover.
- Next free decision ID remains **D-381**.

### DR-20 — 2026-09-24

- Promoted **D-378** from PROPOSED → **CURRENT**: Checkout Fulfilment Mode + Pickup Execution
  Boundary (ADR-018 Accepted; ARCH-R22 / ARCH-G28). Preserves corrected Financial Document Option A
  (Pickup recipient fields null unless immutable accepted recipient authority exists; no invented
  Delivery destination; no Pickup-location-as-recipientAddress; no mutable customer-profile
  reconstruction; `NO_NEW_LEGAL_CLAIM`).
- Capability architecture locked at
  ...
  after independent Architecture Fit PASS (PR #239 review `5295149318`; evaluated HEAD
  `aab814c238c499367ee921e9f8ffb03ff7b1b373`).
- Does **not** authorize IMP-036H implementation, start implementation, execute schema migration,
  activate IMP-036I / IMP-039 / IMP-040, or accept IMP-036H / IMP-037 / IMP-038.
- Next free decision ID remains **D-379**.
- **Architecture Fit candidate reservation (2026-09-25; same DR-20 tip):** Registered **D-379** as
  **PROPOSED** (Scheduled Fulfilment Timing + Future Execution Boundary) with Proposed ADR-019 and
  proposed ARCH-G29 / ARCH-R23 lock delta recorded in the IMP-036I capability Fit candidate. Does
  **not** promote D-379 to CURRENT, does **not** bump register version, does **not** advance
  ARCHITECTURE meta beyond ARCH-R22, does **not** claim Architecture Fit PASS, does **not** amend
  D-378 while D-379 remains PROPOSED, and does **not** authorize implementation. Next free decision
  ID advanced to **D-380**.
- **Delivery finality candidate reservation (2026-09-25; same DR-20 tip):** Registered **D-380** as
  **PROPOSED** (Delivery Successful-Completion Finality + Replacement Boundary) with Proposed
  ADR-020 and proposed ARCH-G30 / shared ARCH-R23 lock delta recorded in the IMP-036I capability Fit
  candidate after human architecture direction APPROVED 2026-09-25. Preserves **D-379** =
  **PROPOSED**. Does **not** promote D-380 (or D-379) to CURRENT, does **not** bump register version
  to DR-21, does **not** advance ARCHITECTURE meta beyond ARCH-R22, does **not** claim Architecture
  Fit PASS, does **not** rewrite ADR-011 historical classification, does **not** change runtime
  Delivery operations, and does **not** authorize implementation. Next free decision ID advanced to
  **D-381**.
...
- Activation authority remains D-377 at GTM-R141 / STATE-R139 (`STATE-R139 = IMP036H_PRODUCT_DEFINITION_ACTIVATION`). CURRENT tip after Product Definition Gate PASS is GTM-R142 / STATE-R140 (`STATE-R140 = IMP036H_PRODUCT_DEFINITION_GATE_PASS`) without changing D-377 decision text meaning.
- **Architecture Fit candidate reservation (2026-09-23; same DR-19 tip):** Registered **D-378** as
  **PROPOSED** (Checkout Fulfilment Mode + Pickup Execution Boundary) with Proposed ADR-018 and
  proposed ARCH-G28 / ARCH-R22 lock delta recorded in the IMP-036H capability Fit candidate. Does
  **not** promote D-378 to CURRENT, does **not** bump register version, does **not** advance
  ARCHITECTURE meta beyond ARCH-R21, does **not** claim Architecture Fit PASS, and does **not**
  authorize implementation. Next free decision ID advanced to **D-379**.

### DR-18 — 2026-09-23

- Registered **D-376**: IMP-038 CSP Maps Fonts inventory amendment — exact hosts
  `fonts.googleapis.com` (`style-src`) and `fonts.gstatic.com` (`font-src`) only; no wildcards;
  no ARCH-R22. Narrowly amends locked IMP-038 §8.2 under ARCH-R21 / D-375 / ADR-017.
- Does **not** accept IMP-038, flip CSP Enforce by itself, activate IMP-039, or broaden Google
  host families beyond the two named directive-specific hosts.
- Next free decision ID advanced to **D-377**.
- **Founder ratification (2026-09-23):** D-376 / DR-18 explicitly ratified. Authorized CSP additions
  remain exactly `style-src: https://fonts.googleapis.com` and `font-src: https://fonts.gstatic.com`.
  No Google wildcards; no ARCH-R22; no new service; no role/permission change; no IMP-039 activation.
  Resolves authority ambiguity from implementation preceding explicit Founder RED ratification.
  Does **not** reopen `GAP-CSP-ENFORCE-001`.
- **Register structure repair (2026-09-23):** Restored malformed D-375/D-376 Current Global Decisions
  table rows to the eight-column schema (ID / Title / Scope / Status / Record / Supersedes /
  Superseded By / Governs). D-376 semantics unchanged; next free ID remains **D-377**.

### DR-17 — 2026-09-22

- Registered **D-375**: Edge / Origin Trust / Application-Authoritative Security Hardening —
  Cloudflare Free supplemental edge; origin trust chain (AOP + Full strict + DO firewall allowlist);
  Nginx CSP/real_ip authority; Turnstile challenge; auth-abuse extension + workforce step-up
  (ADR-017). Capability architecture locked at
  [`capabilities/IMP-038-security-privacy-hardening.md`](./capabilities/IMP-038-security-privacy-hardening.md).
- Global architecture → ARCH-R21 (ARCH-G27). Does **not** authorize IMP-038 implementation, start
  implementation, provision Cloudflare/DigitalOcean resources, or activate IMP-039.
- Next free decision ID advanced to **D-376**.
...
- Registered **D-373**: Initial Administration API uses a dedicated workforce-admin transport hosted
  on the existing operations process (`/api/admin/v1/*`; no new deployable service).
- Locked shared trusted workforce-session → principal construction for administration, mirroring
  D-372 trust rules without expanding Operations Console Order scope.
- Global architecture → ARCH-R19 (ARCH-G25). Capability architecture locked at
  [`capabilities/IMP-035-initial-administration-capabilities.md`](./capabilities/IMP-035-initial-administration-capabilities.md).
- Next free decision ID advanced to **D-374**.

### DR-14 — 2026-08-24

- Registered **D-372**: Operations Console API uses a dedicated workforce business transport backed
  by shared trusted workforce-session authority.
- Locked `/api/operations/v1/*` as the distinct workforce-business transport boundary. Existing
  workforce session/identity eligibility, trusted principal construction, permission/scope
  authorization, and Order authority remain binding; no second authentication system, role-name
  bypass, caller-manufactured authority, or mandatory workforce-auth HTTP hop is permitted.
- V1 is limited to Order search/list/detail and ACCEPT / FULFIL / CANCEL. Existing Order lifecycle,
  expected-order-revision concurrency, replay, and durable lifecycle attribution remain unchanged.
  Refund, Financial Document, RefundStatutoryDecision, SignatureArtifact, generic business audit,
  delivery, notifications, and Operations Console UI remain out of scope.
- Global architecture → ARCH-R17 (ARCH-G23). Capability architecture locked at
  [`capabilities/IMP-029-operations-console-api.md`](./capabilities/IMP-029-operations-console-api.md).
  IMP-029 implementation remains NOT_AUTHORIZED / NOT_STARTED; this decision creates no runtime,
  route, schema, migration, permission-catalog, cookie, or deployment change.
- Next free decision ID advanced to **D-373**.

### DR-13 — 2026-08-21

- Registered **D-371**: Durable Cart Unit Sequence Authority.
- Locked the existing coalesced configured Cart-line model plus an internal, durable, immutable,
  server-issued per-unit add ordinal. The transactional invariant is active unit-record count per
  line equals that line's quantity.
- Locked server-owned latest-active-unit selection for product-level `variantId` decrement and the
  corresponding specific-line, clear, expiry, deletion, identity-transition, and existing Cart
  lock/revision requirements. The browser never selects the configuration line for card minus.
- D-371 layers on D-369 configuration intent and D-370 identity transition; it does not supersede
  either. D-370 reconciliation preserves immutable ordinals while moving source records atomically.
- Requires a forward-only migration. Pre-D-371 per-unit history is not reconstructible, so
  authoritative FILO/LIFO starts with Cart state established under the new invariant.
- Global architecture → ARCH-R16 (ARCH-G22). The IMP-028D RC3 capability amendment authorizes
  future bounded implementation only; this decision creates no implementation, migration, route,
  or runtime change.
- Next free decision ID advanced to **D-372**.
