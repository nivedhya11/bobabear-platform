...
- New decisions continue after the highest CURRENT/AMENDED/PROPOSED register ID: next ID **D-381**
  (**D-379** remains **PROPOSED** for IMP-036I Scheduled Fulfilment Timing + Future Execution
  Boundary; **D-380** is reserved as **PROPOSED** for Delivery Successful-Completion Finality +
  Replacement Boundary; neither is CURRENT until Architecture Fit lock persistence).
...
| ADR-018 | Customer fulfilment mode and Pickup boundary | CURRENT | Checkout Snapshot owns immutable `DELIVERY` \| `PICKUP`; Pickup fail-closed vs Delivery; Financial Document Option A; **D-378**; ARCH-G28 / ARCH-R22 |
| ADR-019 | Scheduled fulfilment timing and execution boundary | PROPOSED | Checkout/Snapshot timing ownership `ASAP` \| `SCHEDULED`; no Slot capacity engine; sealed cutoff; IMP-032 manual Delivery preserved; notification reminder via owned outbox; **D-379** PROPOSED; reminder Delivery completion depends on human-approved **D-380** / ADR-020 candidate (not yet CURRENT); ARCH-G29 / ARCH-R23 proposed lock delta only — not CURRENT until Architecture Fit lock |
| ADR-020 | Delivery successful-completion finality | PROPOSED | `DELIVERED` durable successful execution truth; no normal replacement after `DELIVERED`; FAILED/CANCELLED replacement only under existing accepted prerequisites; history-preserving correction outside IMP-036I; **D-380** PROPOSED; ARCH-G30 / ARCH-R23 proposed lock delta only — not binding until independent Fit / architecture lock persistence |
