...
- New decisions continue after the highest CURRENT/AMENDED/PROPOSED register ID: next ID **D-379**
  (**D-378** is reserved as **PROPOSED** for IMP-036H Checkout Fulfilment Mode + Pickup Execution
...
| ADR-018 | Customer fulfilment mode and Pickup boundary | PROPOSED | Checkout Snapshot owns immutable `DELIVERY` \| `PICKUP`; Pickup fail-closed vs Delivery; **D-378** PROPOSED; ARCH-G28 / ARCH-R22 proposed lock delta only — not CURRENT until Architecture Fit lock |
