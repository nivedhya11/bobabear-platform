...
| T-10 | XSS / script injection | Public static + admin | CSP allowlist (Enforce) + nosniff | CSP reports / browser violations | Enforce; fix allowlist | `docker/nginx/security-headers.conf`; nginx header test; csp-header-evidence | HIGH — Enforce active after D-376 Maps Fonts journey proof | Platform ops |
| T-11 | Clickjacking | Framed storefront/admin | `frame-ancestors 'self'` + XFO SAMEORIGIN | CSP reports | Enforce | Same CSP/header evidence | LOW (headers present; CSP Enforce) | Platform ops |
