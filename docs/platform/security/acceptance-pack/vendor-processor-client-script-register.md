...
| Google Maps / Places | Auth-gated location UX | Browser Maps JS + server key boundary | `https://maps.googleapis.com`; `https://maps.gstatic.com` | Geocode/Places under auth gate; no default raw telemetry retention | Location | `tests/imp-036b/maps-security.test.ts`; CSP |
